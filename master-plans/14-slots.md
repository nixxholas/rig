...
agents and the app use, serves applet content, and pushes changes to the
...
2. Button — an action: send a message to the current chat, open an applet
...
Applets live in an applet folder in the user data — the `<home>/Happy`
folders — and rig serves them the way the html preview is served. An applet is
created by importing a folder into the app; no agent writes into the applet
folder directly. Creating one provides a human-readable kebab-case name, a
...
on. An applet can be reverted to a specific version, which becomes current
...
**B. Applets.** The `<home>/Happy/Applets` folder, creating an applet by
...
update, and remove slot entries and to create an applet.
...
- An applet imported by name under `<home>/Happy` is served over HTTP with
  path traversal rejected and only the current version's files reachable;
  importing again creates the next version, and reverting makes an old version
  current without deleting any.
