...
Reviewed: 2026-08-15. Scope: `packages/happy-agent-modules/sources/collaboration/` compared against
Rig's agent-to-agent surface (`packages/rig/sources/tools/agents/`,
`packages/rig/sources/agent/tools/claude/Agent.ts`, `SendMessage.ts`), root `AGENTS.md`, and master
plans 00, 07, 12, 16, 20, 21.

## Summary

3,353 lines implementing a durable collaborator roster, message log and reply-obligation system
behind six tools. It is the module whose ambition most exceeds what any master plan asks for: plan 7
describes agents that never die, are reached by unguessable Agent ID shared by hand, and accept
follow-ups — the module delivers that, and then adds a listable roster, directed reply obligations
with a state machine, a capacity protocol, and a per-turn broker round trip during tool assembly.
The durability work is real and the follow-up behavior plan 7 calls a regression is genuinely
restored here.

## How it differs from Rig's equivalents

- **A third naming scheme.** Rig has vendor-shaped `Agent` and `SendMessage`
  (`packages/rig/sources/agent/tools/claude/Agent.ts:23`, `SendMessage.ts:7`) and common
  `agent_info` / `agent_send` / `agent_me` (`packages/rig/sources/tools/agents/`). The module adds
  `create_agent`, `list_agents`, `send_agent_message`, `reply_to_agent_message`, `wait_for_reply`,
  `interrupt_agent` (`CollaborationModule.ts:583-599`). Three vocabularies now exist for one
  capability.
- **Discovery, which Rig deliberately withholds.** Rig's `agent_info` description states outright
  "This cannot list or search for agents" and `agent_send` "cannot discover or list agents and
  rejects targets that were not inspected first"
  ...
  unguessable-ID property is the whole access-control story in plan 7, and a list tool dissolves it;
  the module substitutes an optional `authorization` callback for it (finding 1).
- **Reply obligations have no Rig counterpart.** Rig's model is fire-and-forget steering plus the
  parent's own `Agent`/`SendMessage` lifecycle. The module adds a durable obligation record with
  `pending`/`answered` states, a responder constraint, and a blocking `wait_for_reply`
  (`CollaborationModule.ts:521-582`, `CollaborationMessage.ts`). This is new product surface, not a
  port.
- **Permission behavior matches Rig where it matters.** `send_agent_message` carries the same
  `readOnly` switch as Rig's `agent_send` `read_only` (`tools/send_message.ts:15` versus
  `packages/rig/sources/tools/agents/agent_send.ts:19-24`) and both declare
  `shouldReviewInAutoMode: () => false`. Neither couples review to elevation anywhere in the module
  — no tool here sets `shouldRunInFullAccessInAutoMode`, which is correct and better than the
  applets module manages.
...
1. **As wired, `list_agents` throws whenever the roster holds an agent the caller does not own.**
   ...
   child (`CollaborationModule.ts:1286-1292`); otherwise, with no `authorization` option
   configured, it throws `Agent "<id>" is not authorized to list agent "<id>".`
   (`CollaborationModule.ts:1293-1298`).
   `packages/happy-agent/sources/modules/agent/loadHappyAgent.ts:384-399` constructs the module
   with `broker` and `modelCatalog` only — no `authorization`. So the listing tool fails outright,
   rather than filtering, as soon as a second unrelated agent exists. Authorization denial inside a
   list should be a filter; here it is a fatal error for the whole page.
...
3. **Tool assembly performs a broker round trip and rebuilds a tool description every turn.**
   ...
   caching and makes the model's view of its own capabilities non-reproducible. Capacity is
   dynamic state and belongs in the tool's *result* or in an error, not in its description.
4. **`disabledProviders` is hardcoded empty at the only call site.**
   `loadHappyAgent.ts:397` passes `disabledProviders: []`, so the catalog rendering branch for
   disabled providers (`CollaborationModule.ts:657-661`) is dead in practice while the module
...
5. **Two storage layers, both validating.** `CollaborationStore.ts` (299 lines) defines the storage
   ...
   created.
6. **Another create-then-drop migration pair.** Per the README, `001-collaboration` is immutable and
   still creates a receipt table that `002-drop-collaboration-receipts` removes. Same observation as
   the applets module: the early-stage policy prefers advancing the generation over carrying the
   pair forward, and the module is consumed by exactly one host.
7. **Message and obligation tables have no retention story.** The README lists
   ...
   retained forever in the agent database, which AGENTS.md's retention rule speaks to directly.
...
10. **The permission switch is gated by the module rather than by Auto review.** `readOnly` on send
    ...
    (`tools/send_message.ts:19`, `tools/reply_to_message.ts:19`). Rig's `agent_send` behaves the same
    way, so this is consistent with product precedent rather than a regression — worth stating
...
- **Plan 7's core requirement is met.** A collaborator survives the end of its task, accepts
  ...
  a live regression; this module fixes it.
- **`interrupt_agent` is the correct use of the permission API.**
  `shouldReviewInAutoMode: () => true` with no `shouldRunInFullAccessInAutoMode`, plus a
  `describeAutoPermissionAction` that says exactly what will happen and what will not
  (`tools/interrupt_agent.ts:28-30`). This is the decoupling AGENTS.md asks for, demonstrated in
  the same package where applets gets it wrong.
