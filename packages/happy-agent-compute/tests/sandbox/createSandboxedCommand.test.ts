...
import { createSandboxedCommand } from "../../sources/sandbox/createSandboxedCommand.js";
...
node -e "console.log('\''quoted & safe'\'')"'`,
    ...
    it.runIf(process.platform !== "darwin" && process.platform !== "linux")(
        ...
        async () => {
            const cwd = await mkdtemp(join(tmpdir(), "agent-compute-sandbox-command-"));
    ...
    it.runIf(process.platform === "darwin")(
        "grants workspace and Git writes while protecting declared project files",
        async () => {
            const cwd = await mkdtemp(join(process.cwd(), ".agent-compute-seatbelt-command-"));
            tempDirectories.push(cwd);
            ...
            const result = await createSandboxedCommand({
                ...
                cwd,
                hostPolicy: {
                    protectedProjectFiles: ["agent-state", "agent-cache"],
                },
            ...
            expect(result.args).toContain(`-DPROTECTED_WRITE_0=${join(cwd, "agent-state")}`);
            expect(result.args).toContain(`-DPROTECTED_WRITE_1=${join(cwd, "agent-cache")}`);
        },
    );
});
