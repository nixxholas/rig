...
/**
 * Reserves the container-side view of the managed-network bridge root.
 *
 * The same reserved directory the host owns must exist and be traversable from inside the
 * container, and it must be a real directory rather than a symlink pointing elsewhere. This creates
 * it with write-and-traverse-only permissions and verifies the container user can actually reach
 * it, failing closed otherwise.
 */
