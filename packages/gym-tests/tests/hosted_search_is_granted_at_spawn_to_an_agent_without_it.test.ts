 ...
 * network tool in Rig there is no call left for Rig to intercept. Spawning is how an agent that
 * does not already hold one reaches a provider-run search, and the spawn is the reviewable
 * moment. These check that from the outside: the capability is offered on the spawn, an agent
 * that cannot reach past the sandbox cannot hand one out, and nobody is given one they did not
 * ask for.
 *
 * A root Grok agent is the deliberate exception and holds them without a spawn, because choosing
 * Grok is choosing its backend. The gym runs its own provider rather than Grok, so that case is
 * pinned by `resolveHostedCapabilities.test.ts` instead.
 */
describe("granting a search the provider runs on its own backend", () => {
    ...
    it("offers the capability on the spawn and gives it to no other provider by default", async () => {
        const gym = await createGym({
            cols: 96,
        ...
        // Full access is the widest mode there is, and a provider that does not run search on its
        // own backend still gets none of it without a spawn that granted one.
