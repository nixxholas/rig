...
} from "../../sources/modules/git/GitStateTracker.js";
import type { GitChangeState } from "../../sources/modules/git/types.js";
...
describe("GitStateTracker", () => {
    it("coalesces bursts, publishes changes only, and keeps versions monotonic", async () => {
        ...
        const tracker = new GitStateTracker({
            onSnapshot: (_ctx, _entity, snapshot) => published.push(snapshot),
            rootContext: root,
            runGit: async () => ({
                stdout: "/tmp/repo/.git\n/tmp/repo/.git\n",
