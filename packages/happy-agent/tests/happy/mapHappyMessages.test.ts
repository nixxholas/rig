...
import { HappyMessageMapper } from "../../sources/modules/happy/index.js";
import type { HappySessionEvent } from "../../sources/modules/happy/index.js";
...
function accepted(text: string, extra: Record<string, unknown> = {}): AgentEvent {
...
describe("Happy message mapping", () => {
    it("shows what the person said", () => {
        ...
        const messages = mapper.map(accepted("build me a thing"));
    ...
    it("stays silent about a message that came from the phone", () => {
        const mapper = new HappyMessageMapper();
        const echo = accepted("sent from Happy", {
            metadata: { happy: { remoteMessageId: "remote-1" } },
        });
        expect(mapper.map(echo)).toEqual([]);
    });

    it("stays silent about a message meant to stay out of sight", () => {
        const mapper = new HappyMessageMapper();
        expect(mapper.map(accepted("internal", { metadata: { hideFromUser: true } }))).toEqual([]);
    });
    ...
    it("speaks for the runtime in its own voice", () => {
        ...
        const notice = event("message.accepted", {
            id: "msg-system",
            kind: "send",
            message: { content: [{ text: "A process died.", type: "text" }], role: "system" },
            runId: RUN,
        });
        const messages = mapper.map(notice);
        ...
        const steered = mapper.map(
            event(
                "message.accepted",
                {
                    id: "msg-steer",
                    kind: "steering",
                    message: { content: [{ text: "wait, stop", type: "text" }], role: "user" },
                    runId: RUN,
                },
                1_400,
            ),
    ...
    it("shows the same event only once", () => {
        const mapper = new HappyMessageMapper();
        const message = accepted("hello");
        expect(mapper.map(message)).toHaveLength(1);
        expect(mapper.map(message)).toEqual([]);
    });
