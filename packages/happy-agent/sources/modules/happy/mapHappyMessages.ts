...
const contentBlockSchema = Type.Object(
    { text: Type.Optional(Type.String()), type: Type.String() },
            ...
            Type.Object(
                {
                    happy: Type.Optional(
                        Type.Object(
                            { remoteMessageId: Type.Optional(Type.String()) },
                            { additionalProperties: true },
                        ),
                    ),
...
export class HappyMessageMapper {
    ...
    map(event: AgentEvent): readonly HappySessionProtocolMessage[] {
        if (this.#appliedEventIds.has(event.id)) return [];
        this.#remember(event.id);

        if (event.type === "message.accepted") return this.#mapAccepted(event);
    ...
    #mapAccepted(event: AgentEvent): readonly HappySessionProtocolMessage[] {
        ...
        // The phone already shows what it sent; echoing it back would double it.
        if (accepted.metadata?.happy?.remoteMessageId !== undefined) return [];
