...
/**
 * Mint a UUIDv7 that is strictly newer than the version it replaces.
 *
 * Persisting the high-water mark in the resource itself keeps ordering correct across daemon
 * restarts and wall-clock rollback without introducing another singleton or clock service.
 */
export function createProfileVersion(previous?: string, now: () => number = Date.now): string {
...
function parseVersion(value: string): { readonly randomTail: bigint; readonly timestamp: bigint } {
    if (!UUID_V7_PATTERN.test(value)) {
        throw new Error("The stored profile version is invalid.");
    }
