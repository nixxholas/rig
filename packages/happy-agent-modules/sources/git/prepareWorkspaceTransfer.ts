...
import { createHappyIgnore, type HappyIgnore } from "./HappyIgnore.js";

export type WorkspaceTransferState =
    | { status: "validating" }
    | { commit: string; status: "staging" | "applying" | "prepared" | "committing" }
    | { commit: string; status: "rolling_back" | "succeeded" }
    | {
          error: unknown;
          status: "failed";
...
export async function prepareWorkspaceTransfer(options: {
    ...
    const git = options.git ?? directGitCommandRunner;
    ...
    const commit = await runGitCommandOrThrow(git, options.sourcePath, [
        "rev-parse",
        "--verify",
        "HEAD",
    ]);
    const targetCommit = await runGitCommandOrThrow(git, options.targetPath, [
        "rev-parse",
        "--verify",
        "HEAD",
    ]);
    ...
    const stagingRoot = await mkdtemp(join(tmpdir(), "happy-session-transfer-"));
    const payloadPath = join(stagingRoot, "payload");
    let targetTouched = false;
    const restoreTarget = async (): Promise<void> => {
        await runGitCommandOrThrow(git, options.targetPath, ["reset", "--hard", targetCommit]);
        await runGitCommandOrThrow(git, options.targetPath, ["clean", "-d", "-f", "-f", "-x"]);
    };
    ...
    try {
        ...
        targetTouched = true;
        await runGitCommandOrThrow(git, options.targetPath, ["reset", "--hard", commit]);
        await runGitCommandOrThrow(git, options.targetPath, ["clean", "-d", "-f", "-f", "-x"]);
        const tracked = splitNull(
            await runGitCommandOrThrow(git, options.sourcePath, [
                "ls-tree",
                "-r",
                "-z",
                "--name-only",
    ...
    return {
        ...
        async rollback(originalError = new Error("The workspace transfer was rolled back.")) {
            ...
            try {
                ...
                state = { error: failure, status: "failed", target: "restore_failed" };
                await rm(stagingRoot, { force: true, recursive: true }).catch(() => undefined);
                throw failure;
            }
            await rm(stagingRoot, { force: true, recursive: true }).catch(() => undefined);
...
async function assertGitWorkspace(git: GitCommandRunner, path: string): Promise<void> {
    try {
        if (
            (await runGitCommandOrThrow(git, path, ["rev-parse", "--is-inside-work-tree"])) ===
            "true"
        ) {
            return;
        }
    } catch {
        // Replace raw Git errors with the transfer requirement below.
    }
    throw new Error("Session transfer currently requires Git workspaces.");
}

async function readHappyIgnore(root: string): Promise<HappyIgnore | undefined> {
    const contents = await readFile(join(root, ".happyignore"), "utf8").catch((error: unknown) => {
        if (isMissing(error)) return undefined;
        throw error;
    });
    return contents === undefined ? undefined : createHappyIgnore(contents);
}

function isOverlayExcluded(
    exclusions: HappyIgnore | undefined,
    path: string,
    directory: boolean,
): boolean {
    if (path === ".context" || path.startsWith(".context/")) return false;
    if (path === ".git" || path.startsWith(".git/")) return true;
    return exclusions?.ignores(path, directory) === true;
}
