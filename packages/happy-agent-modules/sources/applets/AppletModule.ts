...
import {
    APPLETS_MIGRATION_KEY,
    APPLETS_REMOVE_IDEMPOTENCY_MIGRATION_KEY,
    APPLET_PROOFS_TABLE,
    APPLET_RECEIPTS_TABLE,
    APPLET_TABLE,
    createAppletDatabase,
    type AppletDatabase,
...
const appletModuleOptionsSchema = Type.Object(
    {
        /**
         * The install target root. Defaults to the user-visible applets folder
         * (`~/Happy/Applets` on darwin, `~/happy/applets` elsewhere). The layout
         * is `<root>/<name>/v1`, `v2`, ... with the icon files in `<root>/<name>/`
...
export { appletModuleOptionsSchema };
export type AppletModuleOptions = Static<typeof appletModuleOptionsSchema>;
...
/** Coordinates module-owned applet state, source staging, and bounded asset reads. */
export class AppletModule implements AgentModule {
    ...
    readonly #eventIdFactory: NonNullable<AppletModuleOptions["eventIdFactory"]>;
    readonly #clock: NonNullable<AppletModuleOptions["clock"]>;
    ...
    readonly migrations: readonly AgentModuleMigration[] = [
        [
            ...
            async (_ctx, database) => {
                await agentDatabaseRun(
                    ...
                    sql`CREATE TABLE IF NOT EXISTS ${sql.raw(APPLET_TABLE)} (
                        name TEXT NOT NULL PRIMARY KEY,
                        applet_json TEXT NOT NULL
                    )`,
                );
                await agentDatabaseRun(
                    database as AgentDatabase,
                    sql`CREATE TABLE IF NOT EXISTS ${sql.raw(APPLET_RECEIPTS_TABLE)} (
                ...
                await agentDatabaseRun(
                    database as AgentDatabase,
                    sql`CREATE TABLE IF NOT EXISTS ${sql.raw(APPLET_PROOFS_TABLE)} (
                        operation_id TEXT NOT NULL PRIMARY KEY,
                        value_json TEXT NOT NULL
                    )`,
                );
        ...
        [
            ...
            async (_ctx, database) => {
                await agentDatabaseRun(
                    database as AgentDatabase,
                    sql`DROP TABLE IF EXISTS ${sql.raw(APPLET_RECEIPTS_TABLE)}`,
                );
                await agentDatabaseRun(
                    database as AgentDatabase,
                    sql`DROP TABLE IF EXISTS ${sql.raw(APPLET_PROOFS_TABLE)}`,
                );
            },
        ],
    ];

    constructor(options: AppletModuleOptions) {
    ...
    async import(ctx: Context, input: AppletImportInput): Promise<Applet> {
        ...
        const operation = await this.#operation(ctx, "create", input.operationId);
    ...
    async createForAgent(
        ...
        return await this.importForAgent(ctx, agentId, input, operationId);
    }
    ...
    async updateForAgent(
        ...
        return await this.update(ctx, name, {
            ...input,
            ...(operationId === undefined ? {} : { operationId }),
        });
    }
    ...
    async revert<Result = Applet>(
        ...
        return await this.#runTransaction(ctx, "revert", async (txCtx) => {
            ...
            let event: AppletEvent | undefined;
            if (mutation.changed) {
    ...
    async revertForAgent(
        ...
        return await this.revert(ctx, name, {
            ...input,
            ...(operationId === undefined ? {} : { operationId }),
        });
    }
    ...
    async remove(ctx: Context, name: string, requestedOperationId?: string): Promise<boolean> {
        ...
        const operation = await this.#operation(ctx, "remove", requestedOperationId);
        return await this.#runTransaction(ctx, "remove", async (txCtx) => {
            ...
            if (!mutation.changed) {
                this.#registerRemoval(txCtx, name);
                return { result: mutation.removed };
            }
            if (existing === undefined) {
            ...
            const observedEvent = await this.#observeTransactional(txCtx, event);
            this.#registerRemoval(txCtx, name, observedEvent);
    ...
    async removeForAgent(
        ...
        return await this.remove(ctx, name, operationId);
    }
    ...
    async readAsset(ctx: Context, input: AppletAssetReadInput): Promise<AppletAsset | undefined> {
        ...
        const result = await readAppletAssetFile(
            this.#rootDirectory,
            input.name,
            expectedVersion,
            input.path,
    ...
    async #moveTargetAside(targetPath: string): Promise<string | undefined> {
        ...
        const orphanPath = join(dirname(targetPath), `.orphan-${randomUUID()}`);
        await rename(targetPath, orphanPath);
        return orphanPath;
    }

    async #assertMissingTarget(targetPath: string): Promise<void> {
        try {
            await lstat(targetPath);
        } catch (error) {
        ...
        throw new Error(
            `The applet version directory ${JSON.stringify(targetPath)} already exists.`,
        );
    }
    ...
    async #event(
        ...
        if (!Value.Check(appletEventSchema, event)) {
            throw new Error("Applet module created an invalid event.");
        }
...
async function invokeListener(
    listener: AppletModuleListener | undefined,
...
export function assertAppletModuleOptions(value: unknown): asserts value is AppletModuleOptions {
    if (!Value.Check(appletModuleOptionsSchema, value)) {
        throw new Error("Applet module options are invalid.");
    }
}
