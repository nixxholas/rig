...
export type SlotsModuleOptions = Omit<
    Static<typeof slotModuleOptionsSchema>,
    "transaction"
> & {
 ...
 *
 * The module owns validation, durable operation identity, persistence, migrations, and event
 * construction. The scope resolver owns opaque target existence; the publisher owns live
...
export class SlotsModule implements AgentModule {
    ...
    constructor(options: SlotsModuleOptions) {
        ...
        this.#store = createSlotDatabase(options.transaction);
    ...
    async get(ctx: Context, agentId: string, id: string): Promise<SlotEntry | undefined> {
        this.#assertAgentId(agentId);
        this.#assertId(id);
        const raw = await this.#store.transaction(ctx, txCtx =>
            requirePromise(this.#store.get(txCtx, agentId, id), "Slot store get"),
        );
    ...
    async #operation(
        ...
        if (
            ...
            throw new Error("Slots module created an invalid operation.");
        }
        return operation;
    }
    ...
    async #durableOperationId(
        ...
        if (!Value.Check(slotOperationStateSchema, next)) {
            throw new Error("Slots module stored an invalid operation identity.");
        }
        return next.id;
    }
    ...
    async #createId(
        ...
        if (!Value.Check(slotOperationStateSchema, next)) {
            throw new Error("Slots module stored an invalid entry identity.");
        }
        return next.id;
    }
    ...
    async #observe(ctx: Context, event: SlotEvent): Promise<void> {
        if (!Value.Check(slotEventSchema, event) || !isDeepFrozen(event)) {
            throw new Error("Slots module created an invalid unfrozen event.");
        }
    ...
    async #runTransaction<Result>(
        ...
        const run = async (txCtx: Context): Promise<SlotChange<Result>> => {
            const change = await work(txCtx);
            expected = deepFreeze(structuredClone(change));
            return change;
        };
        const raw = await this.#store.transaction(ctx, run);
    ...
    async #writeReceipt(
        ...
        if (!Value.Check(slotOperationReceiptSchema, receipt)) {
            throw new Error("Slots module created an invalid operation receipt.");
        }
...
export function assertSlotsModuleOptions(value: unknown): asserts value is SlotsModuleOptions {
    if (!Value.Check(slotModuleOptionsSchema, value)) {
        throw new Error(
            "Slots module options are invalid; check the resolver, publisher, listener, and callbacks.",
        );
    }
}
