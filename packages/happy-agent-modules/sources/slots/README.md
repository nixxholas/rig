...
Storage, target existence, and live projection all belong to the host: the module validates,
...
import { SlotsModule } from "@slopus/happy-agent-modules";
...
const agent = await Agent.create(ctx, { ...options, modules: [slots] });
```
...
since a slot mutation only ever touches host-owned state the module already validates.
...
- **`create_slot`** — creates one entry. Arguments: `slot`, `content` (`{ type: "text", markdown }`
  ...
  module assigns the entry ID; the model never supplies one. Returns the created `entry`.
...
- **Ownership.** `update`, `reorder`, and `remove` only act on entries the calling agent authored
  (`authorAgentId`); the module refuses cross-agent mutation outright.
- **Durability and idempotency.** Every mutating tool is durable: on retry, the module looks up a
...
All of these live on the `SlotsModule` instance and take the same `(ctx, agentId, ...)` shape the
...
- `create(ctx, agentId, input, options?): Promise<SlotEntry>` — creates an entry. `options.operationId`
  lets a host supply its own durable operation identity instead of the module's default.
...
- `tools(ctx, scope): readonly AnyAgentTool[]` — the `AgentModule` hook that supplies the six tools
...
The module holds essentially no state of its own. Everything durable lives in one of two places:
...
- **The injected `SlotStore` (fully host-owned).** This is not a Rig KV; it's a structural
  ...
  `slotOperationReceiptSchema`, `slotMutationProofSchema`) before the module trusts it, and pages
...
characters, and orderings that must be unique, contiguous, and start at zero. The module revalidates
