...
interrupt, and the module exists so that fact reaches the model's instructions and, where a host
allows it, lets the model change it on the user's explicit request.

The module owns none of this data. A host wires it up with a `PresenceStore` — the object that
actually reads, writes, and resolves the effective value, including how expiry, fallback, and
schedules interact at a given instant. The module contributes validation, operation identity,
...
const agent = await Agent.create(ctx, { ...options, modules: [presence] });
```
...
- **`get_presence`** — no arguments. Reads the store's resolved presence at the module's clock
...
per call: the module allocates one operation ID per tool call (see Storage) and replays the same
...
The module also contributes an instruction rather than a tool: `instructions(ctx, scope)` reads
...
All of these take a `Context` first and are exported on the `PresenceModule` instance. `ctx`
...
- **`read(ctx)`** / **`state(ctx)`** — `state` is an alias of `read`. Returns the store's resolved
  ...
  (the narrow structural interface the module implements) exposes to other consumers.
...
- **`tools(ctx, scope)`** and **`instructions(ctx, scope)`** — the `AgentModule` hooks the agent
...
The module holds no state of its own beyond one small idempotency record per in-flight operation;
...
doesn't match what was stored the first time, the module throws rather than silently applying a
different mutation under a reused ID.

**Operation identity.** When a mutation call does not supply `options.operationId`, the module
...
(default 64, configurable 1–10,000) to the store's `schedule.list`, and the module rejects a
...
transaction are checked to exactly match what the module itself decided, so a store implementation
