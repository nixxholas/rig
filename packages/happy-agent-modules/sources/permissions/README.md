...
const permissions = new PermissionsModule({ reviewer, listener, reviewTimeoutMs: 120_000 });
...
(`afterAgentSettled`), or the process restarts. The mode itself — the thing this module enforces —
is not this module's to persist; it lives on the agent (`scope.agent.permissionMode`) and its
durability is `@slopus/happy-agent-base`'s responsibility, not this module's. Anything durable
about permission decisions — an audit log, a review history — is the host's own responsibility,
built on top of the events the module reports through `PermissionModuleListener`.
