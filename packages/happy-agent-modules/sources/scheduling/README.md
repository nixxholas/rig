...
Every option is optional and exists for tests or for tuning bounds: `clock`, `timers`, `idFactory`,
`eventIdFactory`, `listener`, `maxWaitDuration`, `maxScheduleHorizon`, `maxPageSize`,
`maxOutputCharacters`, `maxMessageLength`, and `onPostCommitError`. The module's tables are its own,
created by its migrations, and every database operation runs on the Drizzle facade from `ctx.db`.
...
24 hours by default, which the tool descriptions say out loud.
...
Transactional and post-commit listeners each receive one detached, deeply frozen event:
`wait_started`, `wait_finished`, `message_scheduled`, `scheduled_message_cancelled`, or
`scheduled_message_delivery_outcome`. Post-commit registration uses stdlib `afterCommit(ctx, …)`,
and a post-commit failure is contained and optionally reported to `onPostCommitError`.
