# Happy Agent Modules and Rig v2

## Goal

Replace Rig's old agent runtime with `@slopus/happy-agent-base` plus a set of
small, configurable modules from `@slopus/happy-agent-modules`.
...
inference, tools, and per-agent module KV. Everything that changes how an
agent behaves should be a module. Rig supplies storage and external services
...
│   └── module-owned tables installed by module migrations
├── AgentSystemLocal
│   ├── configurable happy-agent-modules
│   └── final Rig protocol projection module
├── Rig host services
...
Base changes, module state, and Rig projections can commit atomically. A
module never opens this database itself. Rig supplies `AgentStorage`,
`AgentKV`, and structural stores backed by semantic persistence operations.

## Agent Base 0.0.7 contract

Rig v2 targets published `@slopus/happy-agent-base@0.0.7`.
...
- a mutation's complete read-decide-write sequence happens through the
  AgentSystem's transaction integration;
- there are no module-level in-memory locks;
- transactional and post-commit listeners receive the same stable event;
- post-commit uses stdlib `afterCommit(ctx, callback)` and means after the
  outermost host transaction;
- tools call the same public module operations as the host;
- tool retries reuse module-owned durable identities;
- model-facing output is bounded but complete enough to use;
- common tools are provider-neutral.

Modules should resemble `GoalModule`: a small public API, a clear storage
...
The Rig protocol projection module is always last in the module array. It
...
History is the first end-to-end proof that Agent Base persistence, a module
...
- Store all retry/failure/continuation state in module KV, not maps.
- Expose the same operations to host and common tools.
- Do not depend directly on History or Tasks.

### 5. Tasks

Status: complete, reviewed, committed, and integrated with Agent Base 0.0.7.
...
- Record cross-module gaps in `debt/` rather than importing Collaboration.
...
- Accept an opaque project reference without importing ProjectsModule.
- Keep worktrees, Git, paths, and compute in Rig.

## Module-owned storage

Each durable module owns its tables and ordered migration tuples. Agent Base
runs those migrations before startup and passes the active Drizzle database or
transaction facade to every module hook. Modules query their own tables
directly and use `kv`, `sharedKV`, and `runKV` for scoped state.

Host integration rules:

- migrations remain immutable once released;
- new module schema changes append new keyed module migrations;
- modules import no Rig schema or persistence implementation;
- store reads filter and page in SQL before deserializing;
- post-commit publication uses stdlib `afterCommit`;
- external files, processes, network clients, clocks, and policy remain
  injected host resources;
- a module never opens its own database or transaction.
...
clear unavailable/no-op result until a module exists. Unsupported behavior
must not fall back to the old agent runtime.

Compute and permission implementations that already exist in the module
package are not part of Rig v2 wiring in this phase.

## Old runtime removal

Delete obsolete runtime code only after its callers use Agent Base/modules.
...
- Route module APIs through shared module instances.
...
Modules can be built concurrently when their files and host adapters do not
overlap. The dependency-oriented order is:

1. shared guidelines and Agent Base 0.0.7 host identity;
...
Cross-module dependencies do not block an individual module. Inject a
...
9. The finished module is committed in one coherent green commit and pushed
   to `rig-v2`.

Do not use CodeRabbit unless Steve explicitly requests it. Do not stage or edit
master plans as part of module commits. Do not combine unrelated module
changes in one commit.

## Definition of done for a module

A module is finished when:
...
- all durable state is in supplied KV or module-owned tables;
- no authoritative in-memory state or module lock exists;
...
- a separate Luna Max review and final Sol High review have no blocking
  findings;
- the module has its own green commit.

## Migration definition of done

Rig v2 is complete when:

- app and daemon start with Agent Base 0.0.7;
...
- agent and message IDs/metadata survive process restart without heap-only
  routing;
- every supported agent capability is a configurable module;
