# Happy Agent Module Guidelines

Use these rules for every module in this package. They capture mistakes found
while building and reviewing the first Rig v2 modules.

## Module boundary

- Build one shared module instance for an `AgentSystem`. Put per-agent durable
  state in the supplied module `AgentKV`, collection state in `sharedKV`, and
  in-progress state in `runKV`.
- Agent Base scopes Agent KV, `sharedKV`, and `runKV` by module name. Keys
  must be unique within one module, but two different modules may safely use
  the same literal key; do not add a second cross-module namespace or infer a
...
- A module never opens a database. It receives the AgentSystem's Drizzle
  facade, owns its ordered migrations and tables, and uses the supplied
...
- Shape validation is not semantic validation. A host result must also match
  ...
  invariants before the module stores, projects, or announces it.
...
- Revalidate a mutation result after the injected transaction returns. A
  ...
  result; the outer module boundary must still enforce the requested identity,
...
- A validated factory function does not validate the object it returns. Define
  ...
  returns as `Type.Any()`.
- If the module's public contract requires a host capability, make that
...
- Do not add module-level in-memory locks. Perform the whole read-decide-write
  mutation through the AgentSystem transaction and active module database
  facade. If an external system needs coordination that a database transaction
  cannot provide, its injected host adapter owns that coordination.
...
- “Post-commit” means after the host's outermost transaction commits, not merely
  after a nested module operation returns. Register it with stdlib
...
- Trusted module-hook failures do not need elaborate recovery machinery.
...
- Define tool-only input schemas when the host mutation schema contains an
  operation ID, record ID, author, or other module-owned field. Reusing the
  host schema can silently expose those persistence identities to the model.
- Call-scoped KV belongs to one durable tool call, not to a module-wide public
...
- Provider call IDs are correlation data, not globally unique module record
  identities. Allocate a module-owned durable ID.
- Thin tools call the same public operations used by the host. Tool results and
  model-facing text must include the bounded detail required to use the module;
...
- Give every sent or steered message a stable caller-owned ID. Put immutable
  module and protocol correlation in message metadata, then route accepted
...
- Treat message metadata as persisted correlation, not authorization. Validate
  a module-owned TypeBox shape before using fields from the open metadata
  record.
...
- Pass the shared transaction context into agent creation, send, and steer when
  module or host projection must commit with the Agent Base operation.
- Prefer Agent Base's idempotent persisted identity boundary over a duplicate
  module receipt table. Add a host repeat-key table only when it carries a
...
- Store contracts perform bounded paging, filtering, and listing at the storage
  boundary. Never require a host to load an entire archive so the module can
  slice it afterward.
...
- Rig adapters stay thin. SQL and query-builder code belongs in semantic
  persistence operations under Rig's persistence tree, not in the module
  adapter.
...
- Put protocol projection after every configurable module in the host module
...
Every stateful module should cover the applicable cases:

- public methods and tools use the same behavior;
- persistence survives a fresh module instance or process-style reload;
...
- A Luna Max implementation pass owns the module and its focused tests.
...
- Give each delegated implementation or review a self-contained narrow brief
  ...
  into a reviewer; ambient module chatter can be mistaken for a replacement
...
- A distinct Sol High review is the final release gate. Sol reviews only after
  the Luna implementation and Luna review agree the module is ready.
- Feed every recurring or generalizable review mistake back into this file
  before starting the next module.
- Commit a module only after its final Sol review has no blocking findings.
  Keep the commit coherent and limited to the module, its tests, exports,
