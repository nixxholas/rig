...
import { CollaborationModule } from "../../sources/collaboration/CollaborationModule.js";
import {
...
import {
    CollaborationModule as RootCollaborationModule,
...
const baseCtx = createRootContext().named("happy-agent-modules-collaboration");
...
function module(
    ...
    let sequence = 0;
    return new CollaborationModule({
        roster: host,
        store: host,
        broker: host,
...
async function createRoot(
    moduleInstance: CollaborationModule,
    id: string,
): Promise<CollaborationAgent> {
    return await moduleInstance.createAgent(baseCtx, id, {
...
async function createChild(
    ...
    return await moduleInstance.createAgent(baseCtx, ownerAgentId, {
        operationId: `create-${id}`,
        id,
...
describe("CollaborationModule", () => {
    it("exports the module, contracts, and common tools from the package root", () => {
        expect(RootCollaborationModule).toBe(CollaborationModule);
    ...
    it("uses caller-supplied Agent Base IDs and metadata inside one shared host transaction", async () => {
        ...
        const collaboration = module(host, { events, postEvents });
    ...
    it("rejects cross-owner access unless an injected authorization policy grants it", async () => {
        const host = new Host();
        const collaboration = module(host);
        ...
        const allowed = module(host, { authorize: true });
    ...
    it("rolls back roster, receipt, and events when a host write fails", async () => {
        const host = new Host();
        const events: CollaborationEvent[] = [];
        const collaboration = module(host, { events });
    ...
    it("rejects an answer from the wrong responder", async () => {
        const host = new Host();
        const collaboration = module(host, { authorize: true });
        await createRoot(collaboration, "owner");
    ...
    it("validates target ownership references, parent cycles, and self-parent targets on get", async () => {
        ...
        const missingModule = module(missingHost);
        await createRoot(missingModule, "owner");
        ...
        await expect(missingModule.getAgent(baseCtx, "owner", "target")).rejects.toThrow(
            "missing parent",
        );

        const cycleHost = new Host();
        const cycleModule = module(cycleHost);
        await createRoot(cycleModule, "owner");
        await createChild(cycleModule, "owner", "agentone");
        await createChild(cycleModule, "owner", "agenttwo");
        ...
        await expect(cycleModule.getAgent(baseCtx, "owner", "agentone")).rejects.toThrow(
            "cyclic parentage",
        );

        const selfHost = new Host();
        const selfModule = module(selfHost);
        await createRoot(selfModule, "owner");
        selfHost.agents.set("owner", {
            ...selfHost.agents.get("owner")!,
            parentId: "owner",
        });
        await expect(selfModule.getAgent(baseCtx, "owner", "owner")).rejects.toThrow(
            "parent invariants",
        );
    });
    ...
    it("reduces roster pages before returning so every visible identity can reach the model", async () => {
        const host = new Host();
        const collaboration = module(host, { maxOutputCharacters: 256 });
        await createRoot(collaboration, "owner");
        for (let index = 0; index < 12; index += 1) {
    ...
    it("rejects mutable host substitutions and missing post-create Agent Base configuration", async () => {
        ...
        await expect(createRoot(module(mutatedHost), "mutated")).rejects.toThrow("substituted");

        const configHost = new Host();
        configHost.mutateCreateConfig = true;
        await expect(createRoot(module(configHost), "wrongconfig")).rejects.toThrow(
