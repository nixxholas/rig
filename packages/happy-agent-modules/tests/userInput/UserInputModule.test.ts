...
async function createPending(
    ...
    return await module.ask(ctx, agent, {
        operationId: "ask-op",
...
describe("UserInputModule", () => {
    it("exports a TypeBox request schema and a single durable common tool", () => {
        ...
        const tools = module.tools(root, { agent: { id: agent } } as never);
    ...
    it("asks, waits, answers, and renders the same public result through the tool", async () => {
        ...
        const module = makeModule(store, { events, postEvents });
        const world = agentWorld();
        const ctx = callContext(world.storage);
        const tool = module.tools(ctx, { agent: { id: agent } } as never)[0]!;
        ...
        const answered = await module.answer(root, otherAgent, {
            ...
            return await module.answer(root, agent, {
                operationId: "answer-op",
                requestId: request.id,
                answer: "yes",
            });
        ...
        expect(result).toEqual(await module.get(root, agent, request.id));
        expect(module.formatForModel(result)).toContain("Answered");
    ...
    it("keeps a structured selection visible before long answer prose and exposes detail", async () => {
        const store = new UserInputTestStore();
        const module = makeModule(store, { maxOutputCharacters: 256 });
        const request = await module.ask(root, agent, {
        ...
        const answered = await module.answer(root, agent, {
        ...
        const tool = module.tools(root, { agent: { id: agent } } as never)[0]!;
    ...
    it("allows an authorized agent to mutate and wait for another agent's request", async () => {
        const store = new UserInputTestStore();
        const module = makeModule(store, { authorize: true });
        const request = await createPending(module);
        const waiting = module.wait(root, otherAgent, {
    ...
    it("re-attaches a retried durable call after a fresh module instance", async () => {
        const store = new UserInputTestStore();
        const world = agentWorld();
        const ctx = callContext(world.storage);
        const first = makeModule(store);
        ...
        // Recreate the module while both durable calls are still waiting.
        const second = makeModule(store);
    ...
    it("requires host-facing mutations to carry operation identities", async () => {
        ...
        const module = makeModule(store);
        await expect(
            module.ask(root, agent, {
                question: "Missing identity?",
                context: "The host must identify this call.",
            }),
        ).rejects.toThrow("operation identity");
        const world = agentWorld();
        const moduleScopedContext = withAgentKV(
            root,
            new AgentKV(world.storage.persistence(agent), "module-scoped-user-input."),
        );
        await expect(
            module.ask(moduleScopedContext, agent, {
                question: "Module scope?",
                context: "A module-scoped KV is not a tool call.",
            }),
        ).rejects.toThrow("operation identity");
        const request = await module.ask(root, agent, {
        ...
        const retry = await module.ask(root, agent, {
            operationId: "host-ask-operation",
            question: "Host retry?",
            context: "The receipt binds the request identity.",
        });
    ...
    it("hashes legal large contexts and atomically reuses concurrent host ask identities", async () => {
        ...
        const large = await module.ask(root, agent, {
    ...
    it("records unchanged proof data when ask recovers an existing request", async () => {
        ...
        const module = makeModule(store, { events });
        await expect(
            module.ask(root, agent, {
    ...
    it("rejects malformed persisted rows and keeps broker waits outside transactions", async () => {
        ...
        await expect(module.get(root, agent, "bad")).rejects.toThrow("invalid request");

        store.requests.clear();
        const request = await createPending(module);
        const waiting = module.wait(root, agent, { requestId: request.id, operationId: "wait-op" });
        await vi.waitFor(() => expect(store.calls).toContain("wait"));
    ...
    it("binds substituted request, receipt, and proof identities to the requested operation", async () => {
        ...
        await expect(module.get(root, agent, request.id)).rejects.toThrow("different identity");
        readSpy.mockRestore();
    ...
    it("settles away and timeout outcomes without a host timer", async () => {
        ...
        const awayModule = makeModule(awayStore, { available: false });
        const awayRequest = await createPending(awayModule);
        await expect(
            awayModule.wait(root, agent, {
        ...
        const timeoutModule = makeModule(timeoutStore);
        const timeoutRequest = await timeoutModule.ask(root, agent, {
    ...
    it("publishes one terminal event when concurrent waits are awakened by completion", async () => {
        ...
        const module = makeModule(store, { events, postEvents });
        const request = await createPending(module);
        const firstWait = module.wait(root, agent, {
            operationId: "concurrent-wait-a",
            requestId: request.id,
        });
        const secondWait = module.wait(root, agent, {
        ...
        const completed = await module.complete(root, agent, {
    ...
    it("requires an existing, matching, elapsed deadline for timeout completion", async () => {
        ...
        const noDeadlineModule = makeModule(noDeadlineStore);
        const noDeadline = await createPending(noDeadlineModule);
        await expect(
            noDeadlineModule.complete(root, agent, {
        ...
        await noDeadlineModule.complete(root, agent, {
        ...
        const futureModule = makeModule(futureStore);
        const future = await futureModule.ask(root, agent, {
        ...
        await expect(
            futureModule.complete(root, agent, {
        ...
        const mismatchModule = makeModule(mismatchStore);
        const mismatch = await mismatchModule.ask(root, agent, {
        ...
        await expect(
            mismatchModule.complete(root, agent, {
    ...
    it("rejects persisted timeout rows whose timestamp precedes the deadline", async () => {
        ...
        await expect(module.get(root, agent, "malformed-timeout")).rejects.toThrow(
    ...
    it("includes terminal outcome details needed for follow-up work", async () => {
        ...
        const answeredModule = makeModule(answeredStore);
        const answeredRequest = await createPending(answeredModule);
        await answeredModule.answer(root, agent, {
            operationId: "detail-answer",
            requestId: answeredRequest.id,
            answer: "The final answer.",
        });
        const answeredPage = await answeredModule.getPage(root, agent, answeredRequest.id);
        expect(answeredPage.detail).toContain("Answer: The final answer.");

        const cancelledStore = new UserInputTestStore();
        const cancelledModule = makeModule(cancelledStore);
        const cancelledRequest = await createPending(cancelledModule);
        await cancelledModule.cancel(root, agent, {
            operationId: "detail-cancel",
            requestId: cancelledRequest.id,
            reason: "The decision is no longer needed.",
        });
        const cancelledPage = await cancelledModule.getPage(root, agent, cancelledRequest.id);
        ...
        await timeoutModule.complete(root, agent, {
            operationId: "detail-timeout",
            requestId: timeoutRequest.id,
            outcome: "timed_out",
            deadlineAt: 1_000,
        });
        const timeoutPage = await timeoutModule.getPage(root, agent, timeoutRequest.id);
    ...
    it("publishes identical frozen transactional/post-commit events and contains listener failures", async () => {
        ...
        const failing = makeModule(failingStore, { transactionalFailure: true });
        ...
        const postFailure = new UserInputModule({
    ...
    it("runs post-commit callbacks outside the transaction queue", async () => {
        const store = new UserInputTestStore();
        let module: UserInputModule;
        let nested: Promise<UserInputRequest> | undefined;
        module = new UserInputModule({
            store,
            ...
            listener: {
                onEvent: async (_ctx, event) => {
                    ...
                    nested = module.cancel(root, agent, {
    ...
    it("keeps nested module work and post-commit events inside an outer rollback", async () => {
        const store = new UserInputTestStore();
        const postEvents: UserInputEvent[] = [];
        const module = makeModule(store, { postEvents });
        await expect(
            store.transaction(root, agent, async (txCtx) => {
                await module.ask(txCtx, agent, {
    ...
    it("does not resolve a durable waiter until a terminal write commits", async () => {
        ...
        await expect(
            store.transaction(root, agent, async (txCtx) => {
                await module.complete(txCtx, agent, {
    ...
    it("enforces store page bounds and keeps minimum-budget model pages actionable", async () => {
        ...
        const detailPage = await module.getPage(root, agent, maximumId);
        expect(detailPage.detail.length).toBeLessThanOrEqual(100);
        const detailOutput = module.formatDetailPageForModel(detailPage);
        ...
        expect(module.formatForModel(request)).toContain(maximumId);
        expect(module.formatPageForModel({ requests: [], cursor: "0", limit: 1 })).toContain(
            "Outcome:",
        );
        const formattedOversizedPage = module.formatPageForModel({
        ...
        await expect(module.listPage(root, agent, { limit: 1 })).rejects.toThrow(
    ...
    it("requires the store to return the requested absolute page cursor", async () => {
        ...
        await expect(module.listPage(root, agent, { cursor: "150", limit: 1 })).rejects.toThrow(
    ...
    it("keeps earlier requests reachable with previous page cursors", async () => {
        ...
        const middle = await module.listPage(root, agent, { cursor: "1", limit: 1 });
        expect(middle.previousCursor).toBe("0");
        expect(middle.nextCursor).toBe("2");
        const end = await module.listPage(root, agent, { cursor: "3", limit: 1 });
    ...
    it("serializes answer and cancel so exactly one terminal outcome wins", async () => {
        ...
        const [answer, cancel] = await Promise.all([
            module.answer(root, agent, {
                operationId: "race-answer",
                requestId: request.id,
                answer: "yes",
            }),
            module.cancel(root, agent, {
        ...
        expect((await module.get(root, agent, request.id))?.status).toBe(answer.status);
        const replay = await module.answer(root, agent, {
            operationId: "race-answer",
            requestId: request.id,
            answer: "yes",
        });
    ...
    it("replays cancellation and completion receipts after an intervening opposite transition", async () => {
        ...
        const cancelledRequest = await createPending(module);
        const cancelled = await module.cancel(root, agent, {
            operationId: "cancel-replay",
            requestId: cancelledRequest.id,
            reason: "closed",
        });
        expect((await module.get(root, agent, cancelledRequest.id))?.status).toBe("cancelled");
        await module.answer(root, agent, {
        ...
        const completedRequest = await module.ask(root, agent, {
        ...
        await module.cancel(root, agent, {
            operationId: "cancel-after-complete",
            requestId: completedRequest.id,
            reason: "late close",
        });
        await expect(
            module.complete(root, agent, {
                operationId: "complete-replay",
                requestId: completedRequest.id,
                outcome: "away",
            }),
    ...
    it("enforces options, bounds, paging, detail cursors, model output, and authorization", async () => {
        ...
        expect(answered.status).toBe("answered");
        const page = await module.listPage(root, agent, { limit: 1 });
        expect(page.requests).toHaveLength(1);
        const detail = await module.getPage(root, agent, request.id, {
            limit: 3,
        });
        expect(detail.nextCursor).toBeDefined();
        expect(module.formatDetailPageForModel(detail)).toContain(request.id);
        expect(formatUserInputForModel({ ...answered, id: "x".repeat(128) }, 256)).toContain(
            "x".repeat(128),
        );
        await expect(module.get(root, otherAgent, request.id)).rejects.toThrow("not authorized");

        const authorized = makeModule(store, { authorize: true });
