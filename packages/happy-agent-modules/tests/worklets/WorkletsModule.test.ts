...
import { AgentKV, withAgentKV, type AgentModuleScope } from "@slopus/happy-agent-base";
...
const root = createRootContext().named("worklets-module-test");
...
async function setup(
    ...
    const module = makeModule(catalog, runtime, installRoot, overrides);
    return { catalog, runtime, module, installRoot, srcPing, srcEcho };
}
...
function scopeFor(id: string): AgentModuleScope {
    return { agent: { id } } as AgentModuleScope;
}

describe("WorkletsModule", () => {
    it("installs version 1 with the Data folder and the icon on disk", async () => {
        const { module, installRoot, srcPing } = await setup();
        const installed = await module.install(root, "agent-a", {
            name: "disk-worker",
            sourceRef: srcPing,
    ...
    it("adds v2 on the second install while keeping v1 and Data intact", async () => {
        const { module, installRoot, srcPing, srcEcho } = await setup();
        await module.install(root, "agent-a", {
            name: "kept-worker",
            sourceRef: srcPing,
        ...
        const updated = await module.update(root, "agent-a", "kept-worker", {
            sourceRef: srcEcho,
    ...
    it("reverts by moving the pointer without deleting any version files", async () => {
        ...
        await module.update(root, "agent-a", "revert-worker", {
            sourceRef: srcEcho,
            changeDescription: "Add echo",
        ...
        const reverted = await module.revert(root, "agent-a", "revert-worker", {
            version: 1,
    ...
    it("refuses to revert a version whose on-disk provenance was changed", async () => {
        ...
        await module.update(root, "agent-a", "provenance-worker", {
            sourceRef: srcEcho,
            changeDescription: "Add echo",
        ...
        await expect(
            module.revert(root, "agent-a", "provenance-worker", {
                version: 1,
    ...
    it("rolls the files back when the install fails after staging", async () => {
        const { module, catalog, installRoot, srcPing } = await setup();
        catalog.failProofWrite = true;
        await expect(
            module.install(root, "agent-a", {
    ...
    it("cleans a staged revert when a later catalog write fails", async () => {
        const { module, catalog, installRoot, srcPing, srcEcho } = await setup();
        await module.install(root, "agent-a", {
            name: "revert-proof-failure",
            sourceRef: srcPing,
            operationId: "revert-proof-install",
        });
        await module.update(root, "agent-a", "revert-proof-failure", {
            sourceRef: srcEcho,
            changeDescription: "Add echo",
        ...
        catalog.failProofWrite = true;
        await expect(
            module.revert(root, "agent-a", "revert-proof-failure", {
                version: 1,
    ...
    it("cleans a staged revert when its transactional listener fails", async () => {
        ...
        const module = makeModule(catalog, runtime, installRoot);
        await module.install(root, "agent-a", {
            name: "revert-listener-failure",
            sourceRef: srcPing,
            operationId: "revert-listener-install",
        });
        await module.update(root, "agent-a", "revert-listener-failure", {
            sourceRef: srcEcho,
            changeDescription: "Add echo",
            operationId: "revert-listener-update",
        });
        const rejecting = makeModule(catalog, runtime, installRoot, {
            listener: {
    ...
    it("cleans a staged revert when an outer transaction rolls back", async () => {
        ...
        await module.update(root, "agent-a", "revert-outer-failure", {
            sourceRef: srcEcho,
            changeDescription: "Add echo",
        ...
        await expect(
            catalog.transaction(root, async (txCtx) => {
                await module.revert(root, "agent-a", "revert-outer-failure", {
                    version: 1,
    ...
    it("refuses a symbolic link that escapes the source tree", async () => {
        const { module, installRoot } = await setup();
    ...
    it("keeps a worklet's Data folder when it is removed", async () => {
        ...
        expect(await module.remove(root, "agent-a", "removed-worker", "removed-remove")).toBe(true);
    ...
    it("shares behavior between public methods and common tools", async () => {
        const { module, catalog, runtime, srcPing, srcEcho } = await setup();
        ...
        const tools = module.tools(root, scopeFor("agent-a"));
    ...
    it("persists versions, proofs, and receipts across a fresh module instance", async () => {
        const { catalog, runtime, module, installRoot, srcPing, srcEcho } = await setup();
        await module.install(root, "agent-a", {
            name: "state-worker",
            sourceRef: srcPing,
            operationId: "install-1",
        });
        await module.update(root, "agent-a", "state-worker", {
            sourceRef: srcEcho,
            changeDescription: "Add echo",
            operationId: "update-1",
        });
        const fresh = makeModule(catalog, runtime, installRoot);
    ...
    it("lists and pages a maximum legal worklet record", async () => {
        const { catalog, module } = await setup();
        const maximum = makeMaximumWorklet("maximum-worker");
        catalog.rows.set(maximum.name, clone(maximum));

        const page = await module.listPage(root, "agent-a", { limit: 1 });
        expect(page.worklets).toEqual([maximum]);
        const detailPage = await module.getPage(root, "agent-a", maximum.name, {
            limit: 1,
        });
    ...
    it("keeps receipts bounded across the maximum legal version history", async () => {
        ...
        for (let version = 2; version <= MAX_WORKLET_VERSIONS; version += 1) {
            await module.update(root, "agent-a", "maximum-history-worker", {
                sourceRef: maximumSource,
                changeDescription,
    ...
    it("recovers removal cleanup by replay after the post-commit callback is lost", async () => {
        const { catalog, runtime, module, installRoot, srcPing } = await setup();
        await module.install(root, "agent-a", {
        ...
        await module.remove(root, "agent-a", "cleanup-recovery-worker", "cleanup-recovery-remove");
    ...
    it("keeps catalog receipts and proofs write-once", async () => {
        const { module, catalog, srcPing } = await setup();
        await module.install(root, "agent-a", {
    ...
    it("publishes one stable event only for changed mutations and contains post-commit failures", async () => {
        ...
        const { module, srcPing } = await setup({
            listener: {
        ...
        await module.revert(root, "agent-a", "event-worker", {
            version: 1,
    ...
    it("publishes no post-commit event and rolls back files when an outer transaction rolls back", async () => {
        ...
        await expect(
            catalog.transaction(root, async (txCtx) => {
                await module.install(txCtx, "agent-a", {
    ...
    it("rejects malformed persisted version and operation invariants", async () => {
        ...
        await expect(module.get(root, "agent-a", "broken-worker")).rejects.toThrow("Worklet");
        ...
        await expect(module.get(root, "agent-a", "timestamp-worker")).rejects.toThrow(
            "timestamps",
        );
    });

    it("replays a durable mutation without repeating the catalog or filesystem side effect", async () => {
        const { module, catalog, srcPing, srcEcho } = await setup();
        const first = await module.install(root, "agent-a", {
            name: "replay-worker",
            sourceRef: srcPing,
            operationId: "same-install",
        });
        const mutationsAfterFirst = catalog.mutationCount;
        const second = await module.install(root, "agent-a", {
            name: "replay-worker",
            sourceRef: srcPing,
            operationId: "same-install",
        });
    ...
    it("does not let a replay apply after an intervening opposite transition", async () => {
        ...
        await module.remove(root, "agent-a", "opposite-worker", "remove-transition");
        await expect(
            module.install(root, "agent-a", {
                name: "opposite-worker",
                sourceRef: srcPing,
    ...
    it("replays an install after removal and recreation without touching the new generation", async () => {
        const { module, catalog, srcPing, srcEcho } = await setup();
        const original = await module.install(root, "agent-a", {
            name: "reinstall-replay-worker",
            sourceRef: srcPing,
            operationId: "reinstall-replay-install",
        });
        await module.remove(root, "agent-a", "reinstall-replay-worker", "reinstall-replay-remove");
        await module.install(root, "agent-a", {
            name: "reinstall-replay-worker",
            sourceRef: srcEcho,
    ...
    it("replays removal after a later recreation without deleting the new record", async () => {
        ...
        await expect(
            module.remove(root, "agent-a", "recreated-worker", "recreated-remove"),
        ).resolves.toBe(true);
        await module.install(root, "agent-a", {
            name: "recreated-worker",
            sourceRef: srcEcho,
    ...
    it("rejects removal replay evidence whose before proof names another worklet", async () => {
        ...
        await module.remove(root, "agent-a", "proof-worker", "proof-remove");
        ...
        await expect(
            module.remove(root, "agent-a", "proof-worker", "proof-remove"),
    ...
    it("replays update and revert receipts after later opposite transitions", async () => {
        ...
        const firstUpdate = await module.update(root, "agent-a", "version-replay-worker", {
            sourceRef: srcEcho,
            changeDescription: "Add echo",
            operationId: "version-replay-update-one",
        });
        await module.update(root, "agent-a", "version-replay-worker", {
            sourceRef: srcPing,
            changeDescription: "Change back",
            operationId: "version-replay-update-two",
        });
        await expect(
            module.update(root, "agent-a", "version-replay-worker", {
                sourceRef: srcEcho,
                changeDescription: "Add echo",
                operationId: "version-replay-update-one",
            }),
        ...
        const firstRevert = await module.revert(root, "agent-a", "version-replay-worker", {
            version: 1,
            operationId: "version-replay-revert-one",
        });
        await module.revert(root, "agent-a", "version-replay-worker", {
            version: 2,
            operationId: "version-replay-revert-two",
        });
        await expect(
            module.revert(root, "agent-a", "version-replay-worker", {
                version: 1,
                operationId: "version-replay-revert-one",
            }),
    ...
    it("bounds logs, invocation payloads, and declared operations", async () => {
        ...
        const page = await module.readLogs(root, "agent-a", "bounded-worker", {
            limit: 1,
            maxLineCharacters: 5,
            maxCharacters: 10,
        });
        ...
        await expect(
            module.readLogs(root, "agent-a", "bounded-worker", { limit: 2 }),
        ).rejects.toThrow("bounds");
        const result = await module.invokeOperation(root, "agent-a", {
            name: "bounded-worker",
            operation: "echo",
            arguments: { value: "ok" },
        });
    ...
    it("rejects deeply nested invocation arguments before schema traversal can overflow", async () => {
        ...
        await expect(
            module.invokeOperation(root, "agent-a", {
                name: "deep-invocation-worker",
                operation: "echo",
                arguments: nested,
            }),
    ...
    it("rejects log pages that skip records or strand continuation", async () => {
        const { module, runtime, srcPing } = await setup();
        await module.install(root, "agent-a", {
        ...
        await expect(
            module.readLogs(root, "agent-a", "invalid-log-worker", { limit: 1 }),
        ...
        await expect(
            module.readLogs(root, "agent-a", "invalid-log-worker", {
                from: "end",
                limit: 1,
            }),
    ...
    it("rejects a middle page returned for an end log request", async () => {
        ...
        await expect(
            module.readLogs(root, "agent-a", "middle-end-worker", {
                from: "end",
                limit: 1,
            }),
    ...
    it("renders runtime states as natural model-facing text", async () => {
        ...
        const detail = await module.get(root, "agent-a", "status-worker");
        expect(detail).toBeDefined();
        expect(module.formatWorkletForModel(detail)).toContain("running and awake");
        expect(module.formatWorkletForModel(detail)).not.toContain("running/awake");
        runtime.statuses.set("status-worker", {
            name: "status-worker",
            state: "asleep",
            at: 501,
        });
        expect(module.formatStatusForModel(await module.status(root, "agent-a", "status-worker")))
            .toContain("sleeping");
    });

    it("enforces bounded catalog and detail cursors", async () => {
        const { module, srcPing } = await setup({ maxPageSize: 1, maxOutputCharacters: 256 });
        await module.install(root, "agent-a", {
            name: "cursor-worker",
            sourceRef: srcPing,
            operationId: "cursor-install",
        });
        await expect(module.listPage(root, "agent-a", { limit: 2 })).rejects.toThrow(
            "cannot exceed",
        );
        const emptyList = await module.listPage(root, "agent-a", { cursor: 10, limit: 1 });
        expect(emptyList.worklets).toHaveLength(0);
        expect(emptyList.previousCursor).toBe(9);

        const detail = await module.getPage(root, "agent-a", "cursor-worker", {
    ...
    it("allows global cross-agent access by default and honors an injected restriction", async () => {
        const { catalog, runtime, module, installRoot, srcPing } = await setup();
        await module.install(root, "owner", {
            name: "private-worker",
            sourceRef: srcPing,
            operationId: "private-install",
        });
        expect((await module.get(root, "other", "private-worker"))?.name).toBe(
            "private-worker",
        );
        const denied = makeModule(catalog, runtime, installRoot, {
            authorization: () => false,
        });
    ...
    it("makes a detail page and minimum list output actionable", async () => {
        const { module, srcPing } = await setup({ maxOutputCharacters: 256, maxPageSize: 2 });
        ...
        const page = await module.listPage(root, "agent-a", { limit: 1 });
        expect(module.formatPageForModel(page)).toContain(name);
        const detail = await module.getPage(root, "agent-a", name, {
            limit: MAX_WORKLET_DETAIL_PAGE_SIZE,
        });
        expect(module.formatDetailPageForModel(detail)).toContain(name);
        expect(detail.worklet).not.toBeNull();
    });
});
