...
import {
    WorkflowsModule as RootWorkflowsModule,
    ...
    workflowModuleOptionsSchema as rootWorkflowModuleOptionsSchema,
...
describe("WorkflowsModule", () => {
    it("exports the complete workflow runtime and tool surface from the package root", () => {
        expect(RootWorkflowsModule).toBe(WorkflowsModule);
        expect(rootWorkflowModuleOptionsSchema).toBe(workflowModuleOptionsSchema);
    ...
    it("scopes reads and mutations to an agent and rejects foreign store results", async () => {
        ...
        await module.launch(ctx(), OWNER, { workflow: "demo", operationId: "run-1" });

        expect(await module.status(ctx(), "agent-2", "run-1")).toBeUndefined();
        expect(await module.list(ctx(), "agent-2", { cursor: 0 })).toEqual({
            agentId: "agent-2",
            cursor: 0,
            runs: [],
            totalRuns: 0,
        ...
        await expect(
            module.cancel(ctx(), "agent-2", { id: "run-1", operationId: "agent-2-cancel" }),
        ).rejects.toThrow("not found");
        await expect(module.wait(ctx(), "agent-2", "run-1")).resolves.toMatchObject({
            agentId: "agent-2",
            status: "unavailable",
        });
        await expect(module.logs(ctx(), "agent-2", { id: "run-1" })).resolves.toMatchObject({
            agentId: "agent-2",
            id: "run-1",
        });

        const foreignRun = run("run-1", "cancelled", { agentId: OWNER });
        const foreignModule = new WorkflowsModule({
            store: {
                ...fixture.store,
        ...
        await expect(foreignModule.status(ctx(), "agent-2", "run-1")).rejects.toThrow(
            "another agent",
        );
        await expect(foreignModule.list(ctx(), "agent-2")).rejects.toThrow("another agent");
        await expect(foreignModule.wait(ctx(), "agent-2", "run-1")).rejects.toThrow(
            "another agent",
        );
        await expect(foreignModule.logs(ctx(), "agent-2", { id: "run-1" })).rejects.toThrow(
            "outside the requested bound",
        );

        const foreignMutationModule = new WorkflowsModule({
            store: {
                ...fixture.store,
        ...
        await expect(
            foreignMutationModule.cancel(ctx(), "agent-2", {
                id: "run-1",
    ...
    it("rejects schema-valid persisted runs with impossible lifecycle timestamps", async () => {
        ...
        await expect(module.status(ctx(), OWNER, "paused")).rejects.toThrow("pause time");
    });
    ...
    it("launches, validates identity, and publishes only after commit", async () => {
        ...
        const result = await module.launch(ctx(), OWNER, {
            workflow: "demo",
            operationId: "run-1",
        });
    ...
    it("reuses operation IDs and rejects mismatched durable retries", async () => {
        ...
        const first = await module.launch(ctx(), OWNER, {
            workflow: "demo",
            operationId: "run-1",
        });
        expect(
        ...
        await expect(
            module.launch(ctx(), OWNER, { workflow: "other", operationId: "run-1" }),
    ...
    it("isolates normalized launch and mutation requests from hostile host mutation", async () => {
        ...
        const launchModule = new WorkflowsModule({
            store: {
                ...launchFixture.store,
        ...
        await expect(launchModule.launch(ctx(), OWNER, launchInput)).rejects.toThrow(
        ...
        const mutationModule = new WorkflowsModule({
            store: {
        ...
        await expect(mutationModule.cancel(ctx(), OWNER, mutationInput)).rejects.toThrow(
            "wrong run",
        );
    ...
    it("requires host operation IDs before allocation or runner mutation", async () => {
        ...
        await expect(module.launch(ctx(), OWNER, { workflow: "demo" })).rejects.toThrow(
            "require an operation ID",
        );
        await expect(module.cancel(ctx(), OWNER, { id: "run-1" })).rejects.toThrow(
            "require an operation ID",
        );
        await expect(module.resume(ctx(), OWNER, { id: "run-1" })).rejects.toThrow(
            "require an operation ID",
        );
    ...
    it("replays a launch from exact receipt and proof after current state is removed", async () => {
        ...
        const replay = await new WorkflowsModule({ store: fixture.store }).launch(ctx(), OWNER, {
            workflow: "demo",
            input: "input",
            operationId: "run-1",
        });
    ...
    it("hashes maximum escaped workflow input into a fixed durable fingerprint", async () => {
        const fixture = storeFixture();
        const module = new WorkflowsModule({ store: fixture.store });
    ...
    it("replays historical mutation results and rejects missing or tampered evidence", async () => {
        ...
        await module.launch(ctx(), OWNER, { workflow: "demo", operationId: "run-1" });
        const cancelled = await module.cancel(ctx(), OWNER, {
            id: "run-1",
            operationId: "cancel-1",
        });
        ...
        await expect(
            module.cancel(ctx(), OWNER, { id: "run-1", operationId: "cancel-1" }),
        ).rejects.toThrow("inconsistent");
        ...
        fixture.proofs.set(key, originalProof);
        const replay = await module.cancel(ctx(), OWNER, {
            id: "run-1",
            operationId: "cancel-1",
        });
    ...
    it("rejects schema-valid replay evidence whose nested runs belong to another agent", async () => {
        const launchFixture = storeFixture();
        await new WorkflowsModule({ store: launchFixture.store }).launch(ctx(), OWNER, {
            workflow: "demo",
        ...
        await expect(
            new WorkflowsModule({ store: launchFixture.store }).launch(ctx(), OWNER, {
                workflow: "demo",
                operationId: "run-foreign-launch",
            }),
        ).rejects.toThrow("another agent");
        ...
        const mutationFixture = storeFixture();
        const mutationModule = new WorkflowsModule({ store: mutationFixture.store });
        await mutationModule.launch(ctx(), OWNER, {
            workflow: "demo",
            operationId: "run-foreign-mutation",
        });
        await mutationModule.cancel(ctx(), OWNER, {
        ...
        await expect(
            new WorkflowsModule({ store: mutationFixture.store }).cancel(ctx(), OWNER, {
                id: "run-foreign-mutation",
                operationId: "cancel-foreign",
            }),
    ...
    it("derives mutation changes from authoritative before and after state", async () => {
        ...
        const noOp = await new WorkflowsModule({ store: fixture.store }).cancel(ctx(), OWNER, {
            id: "run-1",
    ...
    it("rejects malformed store contracts, transaction results, and asynchronous commit registration", async () => {
        ...
        const malformedModule = new WorkflowsModule({ store: malformedTransaction });
        await expect(
            malformedModule.launch(ctx(), OWNER, { workflow: "demo", operationId: "run-1" }),
        ...
        const asynchronousCommitModule = new WorkflowsModule({
        ...
        await expect(
            asynchronousCommitModule.launch(ctx(), OWNER, {
                workflow: "demo",
                operationId: "run-1",
            }),
    ...
    it("requires Promise-returning store methods and validates their resolved values", async () => {
        ...
        const synchronousTransaction = new WorkflowsModule({
            store: {
        ...
        const synchronousRead = new WorkflowsModule({
            store: {
                ...readFixture.store,
        ...
        const malformedRead = new WorkflowsModule({
            store: {
                ...readFixture.store,
        ...
        const mutationFixture = storeFixture();
        mutationFixture.runs.set("run-1", run("run-1"));
        const synchronousMutation = new WorkflowsModule({
            store: {
                ...mutationFixture.store,
        ...
        const malformedProofModule = new WorkflowsModule({
            store: {
        ...
        await expect(
            malformedProofModule.launch(ctx(), OWNER, {
                workflow: "demo",
        ...
        const malformedWriteModule = new WorkflowsModule({
            store: {
        ...
        await expect(
            malformedWriteModule.launch(ctx(), OWNER, {
                workflow: "demo",
    ...
    it("invokes class-backed WorkflowStore methods through their owning object", async () => {
        ...
        const module = new WorkflowsModule({ store: classBacked });

        await expect(module.status(ctx(), OWNER, "run-1")).resolves.toMatchObject({
            id: "run-1",
            agentId: OWNER,
        });
    ...
    it("preserves class-backed factory and clock owners and validates resolved values", async () => {
        ...
        const badEvent = new WorkflowsModule({
    ...
    it("revalidates substituted transaction results for launch and mutation identity", async () => {
        ...
        await expect(
            mutationModule.cancel(ctx(), OWNER, {
                id: "run-1",
    ...
    it("rejects schema-valid substituted cancel and resume transaction events", async () => {
        ...
        await expect(
            module.resume(ctx(), OWNER, { id: "run-1", operationId: "resume-1" }),
        ).rejects.toThrow("substituted event");
    });
    ...
    it("rejects stalled or empty cursor pages", async () => {
        ...
        for (const { page, cursor } of pageCases) {
            ...
            await expect(
                module.list(ctx(), OWNER, cursor === undefined ? {} : { cursor }),
    ...
    it("supports exact forward, backward, end, and beyond-end offset pages", async () => {
        ...
        const module = new WorkflowsModule({ store });

        const latest = await module.list(ctx(), OWNER, { from: "end", limit: 2 });
        ...
        const first = await module.list(ctx(), OWNER, {
        ...
        const beyond = await module.list(ctx(), OWNER, {
        ...
        const latestLogs = await module.logs(ctx(), OWNER, {
            id: "run-1",
            from: "end",
            limit: 2,
        });
        ...
        const beyondLogs = await module.logs(ctx(), OWNER, {
            id: "run-1",
            cursor: Number.MAX_SAFE_INTEGER,
            limit: 2,
        });
        ...
        const empty = new WorkflowsModule({ store: storeFixture().store });
    ...
    it("rejects duplicate, unordered, filtered, or position-substituted pages", async () => {
        ...
        for (const page of [
            ...
            await expect(module.list(ctx(), OWNER)).rejects.toThrow("duplicate or unordered");
        }

        const terminalModule = new WorkflowsModule({
            store: {
                ...fixture.store,
                list: async () => ({
                    agentId: OWNER,
        ...
        await expect(
            terminalModule.list(ctx(), OWNER, { includeTerminal: false }),
        ...
        for (const page of [
            ...
            await expect(module.logs(ctx(), OWNER, { id: "run-1" })).rejects.toThrow(
                "log positions",
            );
        }

        const beyondEnd = new WorkflowsModule({
            store: {
                ...fixture.store,
                list: async () => ({
                    agentId: OWNER,
    ...
    it("enforces paused resume and terminal cancellation semantics without host rewrites", async () => {
        ...
        const resumed = await module.resume(ctx(), OWNER, {
            id: "paused",
        ...
        const runningNoOp = await module.resume(ctx(), OWNER, {
            id: "paused",
        ...
        for (const status of ["completed", "failed", "cancelled", "unavailable"] as const) {
            ...
            const result = await module.cancel(ctx(), OWNER, {
                id,
    ...
    it("accepts wait results only after terminal or unavailable completion", async () => {
        for (const status of ["queued", "running"] as const) {
            ...
            await expect(module.wait(ctx(), OWNER, "run-1")).rejects.toThrow(
        ...
        for (const status of ["completed", "failed", "cancelled", "unavailable"] as const) {
            ...
            await expect(module.wait(ctx(), OWNER, "run-1")).resolves.toMatchObject({ status });
        }
    });
    ...
    it("requires cancel and resume results to preserve identity and settle the requested status", async () => {
        ...
        const wrongStatusModule = new WorkflowsModule({
            store: {
                ...fixture.store,
                cancel: async (_ctx, agentId, input) => ({
                    agentId,
        ...
        await expect(
            wrongStatusModule.cancel(ctx(), OWNER, { id: "run-1", operationId: "op-1" }),
        ).rejects.toThrow("cancelled run");

        fixture.runs.set("run-1", run("run-1", "paused"));
        const wrongIdentityModule = new WorkflowsModule({
            store: {
                ...fixture.store,
        ...
        await expect(
            wrongIdentityModule.resume(ctx(), OWNER, { id: "run-1", operationId: "op-1" }),
    ...
    it("keeps launch replay IDs in the durable tool call scope", async () => {
        ...
        const first = await module.launchForTool(callContext, "agent-1", { workflow: "demo" });
        const replay = await module.launchForTool(callContext, "agent-1", { workflow: "demo" });
    ...
    it("persists and validates the launch receipt before a store retry", async () => {
        ...
        await expect(
            module.launchForTool(callContext, OWNER, {
                workflow: "demo",
                input: "one\r\ntwo",
            }),
        ...
        const replay = await module.launchForTool(callContext, OWNER, {
            workflow: "demo",
            input: "one\ntwo",
        });
    ...
    it("persists and validates cancel and resume receipts before a store retry", async () => {
        for (const operation of ["cancel", "resume"] as const) {
            ...
            const apply = async (id: string): Promise<WorkflowMutationResult> =>
                operation === "cancel"
                    ? await module.cancelForTool(callContext, OWNER, { id })
                    : await module.resumeForTool(callContext, OWNER, { id });
    ...
    it("rejects legacy string operation receipts", async () => {
        ...
        await expect(
            module.launchForTool(withAgentKV(ctx(), kv), OWNER, { workflow: "demo" }),
    ...
    it("keeps generated model receipts separate for each agent", async () => {
        ...
        const ownerRun = await module.launchForTool(ownerContext, OWNER, { workflow: "demo" });
        const otherRun = await module.launchForTool(otherContext, "agent-2", {
            workflow: "demo",
        });
    ...
    it("replays cancellation after later state without a duplicate event", async () => {
        ...
        const stopped = await module.cancelForTool(stopCtx, OWNER, { id: "run-1" });
        fixture.runs.set("run-1", run("run-1", "running", { startedAt: 1, updatedAt: 3 }));
        const replayed = await module.cancelForTool(stopCtx, OWNER, { id: "run-1" });
    ...
    it("consults the durable receipt when the current target already matches", async () => {
        ...
        const cancelledA = await module.cancel(ctx(), OWNER, {
            id: "run-1",
            operationId: "operation-a",
        });
        fixture.runs.set("run-1", run("run-1", "paused", { updatedAt: 3 }));
        const resumedB = await module.resume(ctx(), OWNER, {
            id: "run-1",
            operationId: "operation-b",
        });
        const cancelledC = await module.cancel(ctx(), OWNER, {
            id: "run-1",
            operationId: "operation-c",
        });
        const replayedA = await module.cancel(ctx(), OWNER, {
            id: "run-1",
            operationId: "operation-a",
        });
        ...
        await expect(
            module.resume(ctx(), OWNER, { id: "run-1", operationId: "operation-a" }),
        ).rejects.toThrow("another operation");
        await expect(
            module.cancel(ctx(), OWNER, { id: "other-run", operationId: "operation-a" }),
        ).rejects.toThrow("does not match");
    });
    ...
    it("does not publish a nested mutation after the outer transaction rolls back", async () => {
        ...
        await expect(
            store.transaction(ctx(), OWNER, async (txCtx) => {
                await module.launch(txCtx, OWNER, { workflow: "demo", operationId: "run-1" });
    ...
    it("keeps maximum identities visible within the minimum model budget", async () => {
        ...
        const logText = module.formatLogsForModel({
            agentId: OWNER,
            id,
            cursor: MAX_WORKFLOW_CURSOR - 1,
    ...
    it("bounds pages and logs at the module boundary", async () => {
        const fixture = storeFixture();
        const module = new WorkflowsModule({
            store: fixture.store,
            maxPageSize: 1,
        ...
        await module.launch(ctx(), OWNER, { workflow: "demo", operationId: "run-1" });
        await expect(module.list(ctx(), OWNER, { limit: 2 })).rejects.toThrow("exceeds");
        const logs = await module.logs(ctx(), OWNER, { id: "run-1", limit: 1 });
    ...
    it("rolls back the host mutation and drops post-commit publication on listener failure", async () => {
        ...
        await expect(
            module.launch(ctx(), OWNER, { workflow: "demo", operationId: "run-1" }),
    ...
    it("preserves listener owners and enforces void or Promise<void> results", async () => {
        ...
        const owned = new WorkflowsModule({
    ...
    it("supports status, cancellation, resume, waiting, and bounded logs", async () => {
        ...
        await module.launch(ctx(), OWNER, { workflow: "demo", operationId: "run-1" });
        expect((await module.status(ctx(), OWNER, "run-1"))?.status).toBe("queued");
        expect(
            (await module.cancel(ctx(), OWNER, { id: "run-1", operationId: "cancel-1" })).run
                .status,
        ).toBe("cancelled");
        fixture.runs.set("run-1", run("run-1", "paused", { updatedAt: 3 }));
        expect(
            (await module.resume(ctx(), OWNER, { id: "run-1", operationId: "resume-1" })).run
                .status,
        ).toBe("running");
        fixture.runs.set("run-1", run("run-1", "completed", { updatedAt: 3 }));
        expect((await module.wait(ctx(), OWNER, "run-1")).id).toBe("run-1");
    });
});
