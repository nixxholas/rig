...
**Image generation** — parity. The module owns the account order, the request, the PNG validation,
and the published file; edits accept local paths or recent conversation images; the finished image
goes back to the model. The one thing Agent Base's tool contract cannot express is a named tool
lock, so generations are serialized per agent inside the module instead.
