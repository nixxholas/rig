/**
 * Removes the citation markers OpenAI writes around a server search's sources.
 *
 ...
 * only exist on OpenAI's backend: a server search never returns its results to Rig, so there is
 ...
 */
export function createServerCitationFilter(): (text: string) => string {
    let inMarker = false;
