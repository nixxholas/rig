...
describeLive("Grok native server search live", () => {
    it(
        "runs web_search on the main response with server:true",
        { timeout: 180_000 },
        async () => {
            ...
            const session = await provider.session(`grok-native-web-${Date.now()}`, {
                instructions:
                    "You are a concise assistant. Always use web_search for current facts.",
    ...
    it(
        "runs x_search on the main response with server:true",
        { timeout: 180_000 },
        async () => {
            const credential = await resolveGrokCredential();
            if (credential === null) {
