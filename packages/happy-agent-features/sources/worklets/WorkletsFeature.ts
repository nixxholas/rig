...
import {
    ...
    workletEventSchema,
    workletFeatureListenerSchema,
    type WorkletEvent,
    type WorkletFeatureListener,
...
export const workletFeatureOptionsSchema = Type.Object(
    {
        catalog: workletCatalogSchema,
        ...
        listener: Type.Optional(workletFeatureListenerSchema),
...
export type WorkletFeatureOptions = Static<typeof workletFeatureOptionsSchema>;
...
const INSTALL_OPERATION_KEY = "install.operation";
const UPDATE_OPERATION_KEY = "update.operation";
const REVERT_OPERATION_KEY = "revert.operation";
const REMOVE_OPERATION_KEY = "remove.operation";

const callOperationStateSchema = Type.Object(
    {
        id: workletAgentIdSchema,
...
/**
 * Keep the normalized mutation target beside the durable operation identity.
 * A fingerprint proves equality only when the original request is available;
 * replay validation also needs the target fields themselves.
 */
...
type WorkletOperationRequest =
    | Omit<Extract<WorkletOperation, { kind: "install" }>, "agentId" | "operationId" | "fingerprint">
    | Omit<Extract<WorkletOperation, { kind: "update" }>, "agentId" | "operationId" | "fingerprint">
    | Omit<Extract<WorkletOperation, { kind: "revert" }>, "agentId" | "operationId" | "fingerprint">
    | Omit<Extract<WorkletOperation, { kind: "remove" }>, "agentId" | "operationId" | "fingerprint">;

type FeatureChange = WorkletTransactionChange;
...
/** Coordinates a host-owned worklet catalog with a host-owned runtime. */
export class WorkletsFeature implements AgentFeature {
    readonly name = "worklets";

    readonly #catalog: WorkletCatalog;
    readonly #runtime: WorkletRuntime;
    readonly #installer: WorkletInstaller;
    readonly #authorization: WorkletAuthorization | undefined;
    readonly #idFactory: NonNullable<WorkletFeatureOptions["idFactory"]>;
    readonly #eventIdFactory: NonNullable<WorkletFeatureOptions["eventIdFactory"]>;
    readonly #clock: NonNullable<WorkletFeatureOptions["clock"]>;
    readonly #listener: WorkletFeatureListener | undefined;
    readonly #optionsOwner: WorkletFeatureOptions;
    ...
    readonly #onPostCommitError: WorkletFeatureOptions["onPostCommitError"];

    constructor(options: WorkletFeatureOptions) {
        assertWorkletFeatureOptions(options);
    ...
    async #runTransaction(
        ctx: Context,
        operation: string,
        work: (txCtx: Context) => Promise<FeatureChange>,
    ): Promise<WorkletCatalogMutationResult> {
        let expected: FeatureChange | undefined;
    ...
    async #newEvent(
        ...
        if (!Value.Check(workletEventSchema, event)) {
            throw new Error("Worklet feature created an invalid event.");
        }
    ...
    async #registerStageRollback(
        ...
        try {
            const rollbackRegistration = this.#catalog.onRollback.call(
                this.#catalog,
                ctx,
                (rollbackCtx) => rollbackNow(rollbackCtx),
            );
            assertSynchronousRegistration(rollbackRegistration, "Worklet catalog onRollback");
        } catch (error: unknown) {
    ...
    async #registerStagePostCommit(
        ...
        try {
            const commitRegistration = this.#catalog.afterCommit.call(
                this.#catalog,
                ctx,
                async (postCommitCtx) => {
                    try {
            ...
            assertSynchronousRegistration(commitRegistration, "Worklet catalog afterCommit");
        } catch (error: unknown) {
    ...
    async #registerRemovePostCommit(
        ...
        const registration = this.#catalog.afterCommit.call(
            this.#catalog,
            ctx,
            async (postCommitCtx) => {
                try {
        ...
        assertSynchronousRegistration(registration, "Worklet catalog afterCommit");
    }
...
export function assertWorkletFeatureOptions(
    value: unknown,
): asserts value is WorkletFeatureOptions {
    if (!Value.Check(workletFeatureOptionsSchema, value)) {
        throw new Error("Worklet feature options are invalid.");
    }
}
