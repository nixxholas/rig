...
import type { PermissionEvent, PermissionFeatureListener } from "./PermissionEvent.js";
import type { PermissionReviewDecision, PermissionReviewer } from "./PermissionReviewer.js";

/** What a permissions feature is built with. */
export interface PermissionsFeatureOptions {
    ...
    readonly listener?: PermissionFeatureListener;
 ...
 * nothing: it cannot know what any particular tool touches. This feature is what turns the mode
 ...
 *
 * The mode itself is not this feature's to own or to keep. The agent carries it, makes its changes
 ...
 */
export class PermissionsFeature implements AgentFeature {
    ...
    readonly #listener: PermissionFeatureListener | undefined;
    /** How long a review may take before the action counts as unreviewed. */
    readonly #reviewTimeoutMs: number;
    /** How many refusals in a row end the turn. */
    readonly #refusalsBeforeStopping: number;
    ...
    constructor(options: PermissionsFeatureOptions) {
    ...
    /**
     * Keep the collection the feature is part of. It is what lets a turn drowning in refusals be
     * stopped.
     */
    ...
    readonly afterAgentSettled = (_ctx: Context, scope: AgentFeatureScope): void => {
     ...
     * and whether allowing it means lifting the sandbox for its length. The feature decides only;
