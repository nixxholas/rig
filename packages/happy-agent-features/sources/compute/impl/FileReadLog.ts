...
/** Where the log lives inside the feature's own store for one agent. */
 ...
 * read. Every change runs under that agent's lock, since two tool calls in one turn may both be
 * reading files.
 */
export class FileReadLog {
    /** The feature's durable store for this one agent. */
    readonly #kv: AgentKV;
    ...
    async record(ctx: Context, path: string, mtimeMs: number): Promise<void> {
        await this.#lock.runInLock(ctx, this.#agentId, async (lockCtx) => {
            await this.#kv.update(lockCtx, READS_KEY, (current) => {
     ...
     */
    async assertRead(ctx: Context, fs: ComputeFileSystem, path: string): Promise<void> {
