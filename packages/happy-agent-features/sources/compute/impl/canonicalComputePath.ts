 ...
 */
export async function canonicalComputePath(fs: ComputeFileSystem, path: string): Promise<string> {
    try {
    ...
    try {
        return joinComputePath(await fs.realpath(parent), basenameComputePath(path));
    } catch {
        return path;
    }
}
