...
const root = createRootContext().named("scheduling-feature-test");
...
describe("SchedulingFeature", () => {
    it("schedules and cancels through one shared public implementation", async () => {
        ...
        const feature = makeFeature(store, scheduler, { value: 1_000 }, events);

        const scheduled = await feature.schedule(root, agentId, {
        ...
        await expect(
            feature.cancelSchedule(root, agentId, {
    ...
    it("samples a relative schedule after store latency", async () => {
        ...
        const feature = makeFeature(store, scheduler, now);

        await expect(
            feature.schedule(root, agentId, {
    ...
    it("binds class-backed scheduler methods when the optional scheduler is omitted", async () => {
        ...
        const feature = new SchedulingFeature({
            store,
            clock: () => 1_000,
        });
        await expect(
    ...
    it("reports an interruption using actual elapsed time and reattaches after restart", async () => {
        ...
        const promise = feature.wait(root, agentId, {
            operationId: "wait-op",
            id: "wait-1",
        ...
        const restarted = makeFeature(store, restartedScheduler, now);
        ...
        expect(feature.formatWaitForModel(result)).toContain("3 seconds");
    ...
    it("enforces time bounds and keeps page output actionable", async () => {
        ...
        await expect(
            feature.wait(root, agentId, {
                operationId: "too-long",
                duration: { unit: "hours", value: 1 },
            }),
        ).rejects.toThrow("cannot exceed");
        await expect(
            feature.waitUntil(root, agentId, {
                operationId: "past",
        ...
        const page = await feature.listSchedulePage(root, agentId, { limit: 1 });
        expect(schedulingSchedulePageSchema).toBeDefined();
        expect(feature.formatSchedulePageForModel(page)).toContain("a".repeat(128));
    });
    ...
    it("omits schedule_message when the injected policy forbids it", async () => {
        ...
        const tools = await feature.tools(root, scope);
    ...
    it("accepts every duration unit and rejects malformed or out-of-range timing", async () => {
        ...
        for (const [index, duration] of durations.entries()) {
            ...
            const pending = feature.wait(root, agentId, {
                operationId,
                id: waitId,
                duration,
            });
    ...
    it("rejects a scheduler that substitutes the durable wait start", async () => {
        ...
        const feature = makeFeature(store, scheduler, { value: 1_000 });
    ...
    it("rejects incompatible delivery outcomes and substituted terminal records", async () => {
        ...
        await expect(
            feature.reportDeliveryOutcome(root, agentId, {
    ...
    it("rejects malformed durable state at the store boundary", async () => {
        ...
        await expect(feature.getSchedule(root, agentId, "bad-message")).rejects.toThrow(
    ...
    it("replays schedule and cancel receipts without duplicate mutations or events", async () => {
        ...
        const first = await feature.schedule(root, agentId, request);
        const callCount = scheduler.calls.length;
        await expect(feature.schedule(root, agentId, request)).resolves.toEqual(first);
        expect(scheduler.calls).toHaveLength(callCount);

        const cancelled = await feature.cancelSchedule(root, agentId, {
    ...
    it("serializes a cancellation versus delivery race through the injected transaction", async () => {
        ...
        const final = await feature.getSchedule(root, agentId, scheduled.id);
    ...
    it("requires a distinct delivery authorization and validates retained page cursors", async () => {
        ...
        const ownedByOther = await feature.schedule(root, "agent-b", {
        ...
        const emptyPage = await feature.listSchedulePage(root, agentId, {
            limit: 1,
            cursor: "4",
        });
        expect(feature.formatSchedulePageForModel(emptyPage)).toContain(
    ...
    it("rejects an empty page beyond the beginning without a previous cursor", async () => {
        ...
        const feature = makeFeature(store, scheduler, { value: 1_000 });
        await expect(
            feature.listSchedulePage(root, agentId, { limit: 1, cursor: "1" }),
    ...
    it("enforces the configured schedule horizon when reading durable rows", async () => {
        ...
        await expect(feature.getSchedule(root, agentId, schedule.id)).rejects.toThrow(
            "configured horizon",
        );
        await expect(
            feature.listSchedulePage(root, agentId, { limit: 1 }),
    ...
    it("uses singular units in model-facing durations", () => {
        const store = new InMemorySchedulingStore();
        const feature = makeFeature(store, new InMemorySchedulingScheduler(store), { value: 1_000 });
        ...
        expect(feature.formatWaitForModel(result)).toContain("1 millisecond");
        expect(
            feature.formatWaitForModel({ ...result, endedAt: 1_000, elapsedMs: 1_000 }),
        ).toContain("1 second");
        expect(
            feature.formatWaitForModel({ ...result, endedAt: 1_001, elapsedMs: 1_001 }),
        ).toContain("1 second");
    });
    ...
    it("keeps delivery and cancellation outcomes transactional and reports listener failures", async () => {
        ...
        const delivered = await feature.reportDeliveryOutcome(root, agentId, {
    ...
    it("denies cross-agent scheduling by default and permits it through authorization", async () => {
        const store = new InMemorySchedulingStore();
        const scheduler = new InMemorySchedulingScheduler(store);
        const denied = makeFeature(store, scheduler, { value: 1_000 });
        await expect(
        ...
        const allowed = new SchedulingFeature({
            store,
            scheduler,
            clock: () => 1_000,
            authorization: {
    ...
    it("keeps every returned schedule identity complete at the minimum output budget", async () => {
        ...
        const page = await feature.listSchedulePage(root, agentId, { limit: 1 });
        expect(feature.formatSchedulePageForModel(page)).toContain(id);
    });
    ...
    it("fits a maximum-length middle-page identity at the minimum budget", async () => {
        ...
        const page = await feature.listSchedulePage(root, agentId, {
            limit: 1,
            cursor: "1",
        });
        const text = feature.formatSchedulePageForModel(page);
    ...
    it("reattaches a durable tool identity from call-scoped AgentKV", async () => {
        ...
        const waitTool = (await feature.tools(ctx, scope)).find((tool) => tool.name === "wait");
        ...
        const restarted = makeFeature(store, scheduler, { value: 1_500 });
