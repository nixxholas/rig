...
export const IMPORTED_ICON_SIZE = 512;
export const IMPORTED_ICON_MAX_BYTES = 4 * 1024 * 1024;
...
/** What the icon belongs to in the messages a person reads, and how to raise that failure. */
...
/**
 * Validates an identity icon and derives display artifacts from it.
 *
 ...
 */
export async function createIconArtifacts(
    source: Uint8Array,
    of: IconSubject,
): Promise<IconArtifacts> {
    const png = Buffer.from(source);
    if (png.byteLength > IMPORTED_ICON_MAX_BYTES) {
        throw of.invalid(`The ${of.subject} icon exceeds the 4 MiB limit.`);
    }
    ...
    try {
        image = sharp(png, {
            animated: false,
            failOn: "error",
            limitInputPixels: IMPORTED_ICON_SIZE * IMPORTED_ICON_SIZE,
        });
    } catch {
        throw of.invalid(`The ${of.subject} icon is not a readable PNG image.`);
    }
    let metadata;
    try {
    ...
    if (metadata.format !== "png") {
        throw of.invalid(`The ${of.subject} icon must be a PNG image.`);
    }
    if (metadata.width !== IMPORTED_ICON_SIZE || metadata.height !== IMPORTED_ICON_SIZE) {
        throw of.invalid(`The ${of.subject} icon must be exactly 512 by 512 pixels.`);
    }
    ...
    try {
        ...
        throw of.invalid(`The ${of.subject} icon could not be decoded.`);
    }
    if (
        normalized.info.width !== IMPORTED_ICON_SIZE ||
        normalized.info.height !== IMPORTED_ICON_SIZE
    ) {
        throw of.invalid(`The ${of.subject} icon must be exactly 512 by 512 pixels.`);
    }
