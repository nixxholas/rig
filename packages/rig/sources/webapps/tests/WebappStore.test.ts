...
describe("WebappStore", () => {
    it("atomically imports safe source files, preserving its 512px icon and persisted metadata", async () => {
        ...
        const created = await store.webapps.create({
        ...
        expect(created).toMatchObject({
            iconUrl: "/webapps/dashboard/favicon.png",
            name: "dashboard",
        });
        ...
        await expect(store.webapps.readIcon("dashboard", "png")).resolves.toMatchObject({
    ...
    it("rejects symbolic links in the imported source tree before recording a version", async () => {
        ...
        ).rejects.toBeInstanceOf(WebappInvalidError);
        expect(store.webapps.get("dashboard")).toBeUndefined();
    });
    ...
    it("restores a stranded directory when database creation fails", async () => {
        ...
        store.database.run(
            sql.raw(`
            CREATE TRIGGER reject_webapp_create
            BEFORE INSERT ON webapps
            BEGIN
                SELECT RAISE(ABORT, 'rejected webapp');
            END
        `),
        );
        ...
        ).rejects.toThrow("rejected webapp");
...
function createStore(
    ...
    return { database: opened.database, webapps: store };
}
