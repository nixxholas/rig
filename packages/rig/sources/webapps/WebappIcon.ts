...
export const WEBAPP_ICON_SIZE = 512;
export const WEBAPP_ICON_MAX_BYTES = 4 * 1024 * 1024;

const ICO_SIZES = [16, 32, 48, 64, 128, 256] as const;

export class WebappIconInvalidError extends Error {
    constructor(message: string) {
        super(message);
        this.name = "WebappIconInvalidError";
    }
}

export interface WebappIconArtifacts {
    ico: Buffer;
    png: Buffer;
    thumbhash: string;
}
...
/**
 * Validates the identity icon supplied for a webapp and derives display artifacts from it.
 *
 ...
 */
export async function createWebappIconArtifacts(source: Uint8Array): Promise<WebappIconArtifacts> {
    const png = Buffer.from(source);
    if (png.byteLength > WEBAPP_ICON_MAX_BYTES) {
        throw new WebappIconInvalidError("The webapp icon exceeds the 4 MiB limit.");
    }
    ...
    try {
        image = sharp(png, {
            animated: false,
            failOn: "error",
            limitInputPixels: WEBAPP_ICON_SIZE * WEBAPP_ICON_SIZE,
        });
    } catch {
        throw new WebappIconInvalidError("The webapp icon is not a readable PNG image.");
    }
    let metadata;
    try {
    ...
    if (metadata.format !== "png") {
        throw new WebappIconInvalidError("The webapp icon must be a PNG image.");
    }
    if (metadata.width !== WEBAPP_ICON_SIZE || metadata.height !== WEBAPP_ICON_SIZE) {
        throw new WebappIconInvalidError("The webapp icon must be exactly 512 by 512 pixels.");
    }
    ...
    try {
        ...
        throw new WebappIconInvalidError("The webapp icon could not be decoded.");
    }
    if (normalized.info.width !== WEBAPP_ICON_SIZE || normalized.info.height !== WEBAPP_ICON_SIZE) {
        throw new WebappIconInvalidError("The webapp icon must be exactly 512 by 512 pixels.");
    }
