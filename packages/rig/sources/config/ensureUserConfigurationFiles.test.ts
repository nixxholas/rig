...
describe("ensureUserConfigurationFiles", () => {
    it("creates a commented Happy template and an empty AGENTS.md", async () => {
        ...
        await ensureUserConfigurationFiles({ agentsPath, configPath });
    ...
    it("creates each file exactly once when first-start initialization races", async () => {
        ...
        await Promise.all(
            Array.from({ length: 8 }, () =>
                ensureUserConfigurationFiles({ agentsPath, configPath }),
            ),
        );
