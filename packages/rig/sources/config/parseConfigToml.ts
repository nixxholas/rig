...
export function parseConfigTomlWithUnknownSettings(source: string): ParsedConfigToml {
    ...
    if (!Value.Check(tableSchema, document)) {
        throw new Error("Rig config must contain TOML tables.");
    }
...
function mapDefaults(defaults: DefaultsDocument): NonNullable<PartialRigConfig["defaults"]> {
    return {
...
function mapSettings(settings: SettingsDocument): NonNullable<PartialRigConfig["settings"]> {
    return {
