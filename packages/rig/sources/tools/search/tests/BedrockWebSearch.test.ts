...
describe("bedrock_web_search", () => {
    ...
    it("reads markdown sources from the bounded answer", async () => {
        const session = recordingSession([
            { type: "toolcall_start", callId: "ws_1", name: "web_search", server: true },
            { type: "text_start" },
            {
                type: "text_delta",
                delta: "EKS added Kubernetes version rollbacks. [AWS News](https://aws.amazon.com/blogs/aws/upgrade-amazon-eks-clusters/)",
            },
            { type: "text_end" },
            { type: "done", state: "normal", tokens: { input: 0, output: 0 } },
        ]);
        ...
        expect(result.answer).toContain("EKS added Kubernetes version rollbacks.");
    });
