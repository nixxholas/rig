...
import { modelOpenaiGpt56Luna, modelOpenaiGpt56Terra } from "@slopus/rig-execution";
...
describe("createEncryptedAgentTransportScope", () => {
    it("scopes native ciphertext to each compatible executor provider", () => {
        const cloud = defineProvider({
            id: "codex",
            type: "codex",
            models: [modelOpenaiGpt56Terra, modelOpenaiGpt56Luna],
            stream: () => {
        ...
        const modelFor = (provider: typeof cloud, modelId = modelOpenaiGpt56Terra.id) => {
            const model = provider.models.find((candidate) => candidate.id === modelId);
            if (model === undefined) throw new Error(`Expected ${modelId} in test provider.`);
            return model;
        };
