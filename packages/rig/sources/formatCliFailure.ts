...
import { isRigUserError } from "./RigUserError.js";
...
const UNEXPECTED_TITLE = "Rig stopped unexpectedly.";
...
/**
 * Renders a failure the way the rest of Rig looks: a marked summary line, an actionable hint,
...
export function formatCliFailure(error: unknown, options: FormatCliFailureOptions = {}): string {
    ...
    const expected = isRigUserError(error);
 ...
 * that belong to Rig, written the way the repository refers to them.
 */
