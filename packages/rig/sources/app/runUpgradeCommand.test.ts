...
describe("runUpgradeCommand", () => {
    it("installs the newest Rig beta globally with npm", async () => {
        ...
        expect(spawnProcess).toHaveBeenCalledWith("npm", ["install", "-g", "@slopus/rig@beta"], {
            stdio: "inherit",
        });
    });
    ...
    it("keeps a canary installation on the newest canary", async () => {
        ...
        expect(spawnProcess).toHaveBeenCalledWith("npm", ["install", "-g", "@slopus/rig@canary"], {
            stdio: "inherit",
        });
    });
    ...
    it("reports a failed npm install as a user-facing upgrade error", async () => {
        ...
        await expect(upgrading).rejects.toThrow("Rig could not upgrade itself.");
    });
