...
/** Builds and launches a relocatable Happy local app carrying the current Rig runtime. */
export async function runDesktop(options: RunDesktopOptions): Promise<void> {
    if (process.platform !== "darwin") {
        throw new RigUserError("The Happy desktop app currently builds only on macOS.");
    }

    const desktopRoot = join(getRigHome(), "desktop");
    const releaseDirectory = join(desktopRoot, "release");
    let application = await desktopApplicationResolve(releaseDirectory);

    if (options.skipBuild) {
        if (!application) {
            throw new RigUserError("Rig has no packaged Happy desktop app to launch.", {
                hint: "Run rig desktop once without --skip-build.",
            });
        }
    ...
    if (!application) {
        throw new RigUserError("The Happy desktop build produced no runnable application.");
    }
