...
export async function desktopApplicationStagingPrepare(
    ...
    if (!Value.Check(packageManifestSchema, manifest)) {
        throw new RigUserError("The Happy desktop package manifest is invalid.");
    }
    await writeFile(
        manifestPath,
        `${JSON.stringify(
    ...
    await writeFile(join(happy2Staging, "rig-main.mjs"), desktopApplicationEntrypoint());

    const runtimeBin = join(rigRuntime, "bin");
    await mkdir(runtimeBin, { recursive: true });
    const rigLauncher = join(runtimeBin, "rig");
    const loginShell = join(runtimeBin, "happy2-login-shell");
    await writeFile(rigLauncher, desktopRigLauncher());
    await writeFile(loginShell, desktopLoginShell());
    await chmod(rigLauncher, 0o755);
...
export function desktopApplicationEntrypoint(): string {
    return [
        ...
        "",
        'const runtimeBin = join(process.resourcesPath, "rig-runtime", "bin");',
...
export function desktopRigLauncher(): string {
    return [
        "#!/bin/sh",
        "set -eu",
