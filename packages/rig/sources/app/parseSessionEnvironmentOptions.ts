...
/** A directory shared between the machine running Rig and the container. */
...
export interface SessionDockerEnvironment {
    /** The image Rig starts a fresh container from, when one was chosen. */
    image?: string;
    /** An already running container Rig attaches to, when one was chosen. */
    container?: string;
...
export function parseSessionEnvironmentOptions(
    ...
    for (let index = 0; index < args.length; index += 1) {
        ...
        if (
            ...
            if (value === undefined) {
                throw new RigUserError(`Rig needs a value after ${argument}.`, {
            ...
            if (argument === "--docker-image") image = value;
                ...
                if (separator <= 0) {
                    throw new RigUserError(
                        `Rig cannot read '${value}' as an environment variable.`,
    ...
    if (image !== undefined && container !== undefined) {
        throw new RigUserError(
            "Rig can start a new container from an image or attach to a running one. Choose one.",
            { hint: "Pass either --docker-image or --docker-container." },
        );
    }
    const docker = image !== undefined || container !== undefined;
    if (local && docker) {
        throw new RigUserError("Rig can run this session locally or in a container. Choose one.", {
            hint: "Pass either --local or the --docker options.",
        });
    }
    if (!docker && (Object.keys(environment).length > 0 || mounts.length > 0)) {
        throw new RigUserError("Rig needs to know which container these options belong to.", {
            hint: "Add --docker-image or --docker-container.",
        });
    }
    if (!docker && workingDirectory !== undefined) {
        throw new RigUserError(
            "Rig needs to know which container that working directory belongs to.",
            { hint: "Add --docker-image or --docker-container." },
        );
    }
...
function parseMount(value: string): SessionDockerMount {
    ...
    if (parts.length < 2 || parts.length > 3 || parts.some((part) => part.length === 0)) {
        throw new RigUserError(`Rig cannot read '${value}' as a shared directory.`, {
    ...
    if (mode !== undefined && mode !== "ro" && mode !== "rw") {
        throw new RigUserError(`Rig does not understand the mount option '${mode}'.`, {
