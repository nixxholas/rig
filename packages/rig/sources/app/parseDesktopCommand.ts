...
export function parseDesktopCommand(arguments_: readonly string[]): RunDesktopOptions {
    ...
    for (let index = 0; index < arguments_.length; index += 1) {
        ...
        if (argument === "--happy2-root") {
            ...
            if (!happy2Root) {
                throw new RigUserError("The --happy2-root option needs a directory.", {
                    hint: "Usage: rig desktop --happy2-root /path/to/happy2",
                });
            }
            index += 1;
            continue;
        ...
        throw new RigUserError(`Unknown rig desktop option '${argument}'.`, {
            hint: "Usage: rig desktop [--build-only] [--skip-build | --force-build] [--happy2-root PATH]",
        });
    }

    if (skipBuild && forceBuild) {
        throw new RigUserError("Rig cannot skip and force the desktop build at the same time.", {
