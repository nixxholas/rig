...
/**
 * Where a worklet may reach on the network. A scoped worklet's traffic runs through Rig's managed
...
/** Everything a worklet may touch beyond its own folder and the socket it reaches Rig on. */
...
export const workletSchema = Type.Object(
    {
        ...
        /** What the current version's manifest asks for, and what Rig is enforcing right now. */
        permissions: workletPermissionsSchema,
        ...
        iconUrl: Type.String({
            description: "Rig HTTP path that serves the worklet's persisted icon.",
            minLength: 1,
        }),
