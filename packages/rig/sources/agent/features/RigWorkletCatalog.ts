...
export class RigWorkletCatalog implements WorkletCatalog {
    ...
    async transaction(ctx: Context, work: (ctx: Context) => Promise<unknown>): Promise<unknown> {
    ...
    async list(ctx: Context, query: WorkletListQuery): Promise<WorkletListPage> {
        ...
        const rows = await withDatabase(ctx, this.#database)
            .tx.select({ name: worklets.name })
            .from(worklets)
    ...
    async get(ctx: Context, name: string): Promise<Worklet | undefined> {
        ...
        const row = await scope.tx.select().from(worklets).where(eq(worklets.name, name)).get();
    ...
    async revert(
        ...
        if (changed) {
            await withDatabase(ctx, this.#database)
                .tx.update(worklets)
    ...
    async readReceipt(
        ...
        const row = await withDatabase(ctx, this.#database)
            .tx.select({ json: workletMutationReceipts.receiptJson })
    ...
    async writeReceipt(ctx: Context, receipt: WorkletCatalogMutationReceipt): Promise<void> {
        assertWorkletReceipt(receipt);
        await withDatabase(ctx, this.#database)
            .tx.insert(workletMutationReceipts)
    ...
    async readMutationProof(
        ...
        const row = await withDatabase(ctx, this.#database)
            .tx.select({ json: workletMutationProofs.proofJson })
    ...
    async writeMutationProof(ctx: Context, proof: WorkletCatalogMutationProof): Promise<void> {
        assertWorkletProof(proof);
        await withDatabase(ctx, this.#database)
            .tx.insert(workletMutationProofs)
