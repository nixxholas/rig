...
describe("formatCliFailure", () => {
    it("shows an actionable failure as a summary and a next step", () => {
        const report = formatCliFailure(
            new RigUserError("Rig has no saved sessions yet.", { hint: "Run rig to start one." }),
            { color: false },
        );

        expect(report).toBe(
            ["", "✗ Rig has no saved sessions yet.", "  Run rig to start one.", ""].join("\n"),
        );
    });

    it("keeps an actionable failure free of stack frames even when it has one", () => {
        const failure = new RigUserError("Unknown rig option '--wat'.");
        failure.stack = `Error: Unknown rig option '--wat'.\n    at main (/repo/packages/rig/sources/app/main.ts:12:7)`;
    ...
    it("summarises an unexpected crash with repository-relative frames", () => {
        ...
        failure.stack = [
            ...
            "    at resolveStartupSessionId (/Users/steve/dev/rig/packages/rig/sources/app/resolveStartupSessionId.ts:30:19)",
            "    at process.processTicksAndRejections (node:internal/process/task_queues:104:5)",
            "    at ModuleJob.run (node:internal/modules/esm/module_job:430:25)",
            "    at async runApp (/Users/steve/dev/rig/packages/rig/sources/app/runApp.ts:167:27)",
        ].join("\n");

        const report = formatCliFailure(failure, { color: false });

        expect(report).toContain("✗ Rig stopped unexpectedly.");
        expect(report).toContain("Cannot read properties of undefined (reading 'id')");
        expect(report).toContain("packages/rig/sources/app/resolveStartupSessionId.ts:30");
        expect(report).toContain("packages/rig/sources/app/runApp.ts:167");
    ...
    it("colours the marker only when the caller asks for colour", () => {
        const failure = new RigUserError("Nope.");
