...
export const APPLET_FILE_MAX_BYTES = 10 * 1024 * 1024;

export type AppletFileResult =
    | { type: "file"; contentType: string; data: Buffer }
    | { type: "invalid_path" }
    | { type: "not_found" };

/**
 * Reads one static file from an applet's version folder for serving over HTTP.
 *
 ...
 */
export async function readAppletFile(
    name: string,
    version: number,
    filePath: string,
    environment: NodeJS.ProcessEnv = process.env,
): Promise<AppletFileResult> {
    if (!isValidAppletName(name)) return { type: "invalid_path" };
    ...
    const root = resolve(getAppletsDirectory(environment), name, `v${String(version)}`);
    ...
    try {
        ...
        if (!facts.isFile() || facts.size > APPLET_FILE_MAX_BYTES) return { type: "not_found" };
