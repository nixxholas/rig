...
describe("AppletContextTokenStore", () => {
    it("mints opaque one-time tokens, expires them, and evicts the oldest token at the cap", () => {
        ...
        const first = tokens.mint({ version: 1, applet: "dashboard" });
        const second = tokens.mint({ projectId: "project-1", version: 1, applet: "dashboard" });
        const third = tokens.mint({ sessionId: "session-1", version: 1, applet: "dashboard" });
