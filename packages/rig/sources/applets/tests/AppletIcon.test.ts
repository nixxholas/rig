...
describe("createAppletIconArtifacts", () => {
    it("preserves an exact 512px PNG, derives a stable ThumbHash, and creates a multi-size rounded ICO", async () => {
        ...
        const first = await createAppletIconArtifacts(original);
        const second = await createAppletIconArtifacts(original);
    ...
    it.each([
        ...
        await expect(createAppletIconArtifacts(image)).rejects.toBeInstanceOf(
            AppletIconInvalidError,
        );
    });
});
