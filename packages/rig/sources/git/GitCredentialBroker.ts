...
export class GitCredentialBroker {
    ...
    async #handle(request: IncomingMessage, response: ServerResponse): Promise<void> {
        if (this.#closed) {
            sendError(response, 503, "Git authentication is unavailable.");
            return;
        }
        if (this.#activeRequests >= MAXIMUM_CONCURRENT_GIT_REQUESTS) {
            sendError(response, 429, "Too many Git requests are active.");
            return;
        }
        const authorized = authorizeRequest(request, this.#records, this.#commandLeases);
        if (authorized === undefined) {
            sendError(response, 404, "Git repository not found.");
            return;
        }
...
function commandAuthenticationFor(
    port: number,
    activate: () => GitCommandAuthenticationLease,
...
function authorizeRequest(
    ...
    const validPostService =
        request.method === "POST" &&
        parts.length === 5 &&
        (firstServicePart === "git-upload-pack" || firstServicePart === "git-receive-pack") &&
        url.search.length === 0;
    if (request.method === "POST" && !validPostService) return undefined;
