...
describe("P2P credential snapshot persistence", () => {
    it("replaces providers and advances the owner's durable version atomically", async () => {
        ...
        expect(
            await replaceP2pCredentialSnapshot(
        ...
        expect(
            await queryP2pCredentialSnapshot(
                withDatabase(createTestRootContext(), opened.database),
                owner.instanceId,
            ),
        ).toEqual({
        ...
        expect(
            await prepareP2pCredentialSnapshotVersion(
                withDatabase(createTestRootContext(), opened.database),
                {
                    ownerInstanceId: owner.instanceId,
                    sourceDigest,
        ...
        expect(
            await fastForwardP2pCredentialSnapshotVersion(
                withDatabase(createTestRootContext(), opened.database),
                {
                    ownerInstanceId: owner.instanceId,
                    receiverVersion: 7,
