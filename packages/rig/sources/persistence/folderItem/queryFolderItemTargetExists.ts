        ...
        switch (target.kind) {
            ...
            case "workspace": {
                ...
                return (
                    ...
                    (await projectIsActive(tx, workspace.projectId))
                );
            }
            case "document":
                return (
        ...
        const activeWorkspaceProjects = new Set(
            (workspaceProjectIds.length === 0
                ...
                : await tx
                      ...
                      .where(
                          and(
                              inArray(projects.id, workspaceProjectIds),
...
async function projectIsActive(tx: TX, projectId: string): Promise<boolean> {
    return (
        (await tx
            .select({ id: projects.id })
            .from(projects)
