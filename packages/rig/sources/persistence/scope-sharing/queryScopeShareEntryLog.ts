...
export function queryScopeShareEntryLog(
    tx: TX,
    ...
    const rows = tx
        ...
        .where(
            and(
                ...
                gt(scopeShareEntries.sequence, input.afterSequence),
            ),
        )
        .orderBy(asc(scopeShareEntries.sequence))
    ...
    const remaining = tx
        .select({ sequence: scopeShareEntries.sequence })
        .from(scopeShareEntries)
        .where(
            and(
                eq(scopeShareEntries.shareId, input.shareId),
                gt(scopeShareEntries.sequence, lastSequence),
            ),
        )
        .limit(1)
        .get();
