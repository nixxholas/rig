...
afterEach(async () => {
    ...
    if (originalAppletsDirectory === undefined) delete process.env.HAPPY_APPLETS_DIRECTORY;
    else process.env.HAPPY_APPLETS_DIRECTORY = originalAppletsDirectory;
});
...
describe("slot HTTP protocol", () => {
    ...
    it("creates, lists, updates, and removes entries with typed rejections", async () => {
        ...
        const appletButton = await request(port, {
            body: JSON.stringify({
                author: { type: "agent", sessionId: "session-1" },
                content: {
                    action: {
        ...
        expect(appletButton.status).toBe(201);
        expect(JSON.parse(appletButton.body)).toMatchObject({
            entry: {
                content: {
                    action: {
                        path: "/reports/daily",
...
describe("applet HTTP protocol", () => {
    it("imports versions, serves only the current version safely, and reverts", async () => {
        const port = await startServer();
        const sources = await createTempDirectory("rig-applet-sources-");
        ...
        expect(JSON.parse(badName.body)).toMatchObject({ error: { code: "invalid_applet" } });
        ...
        const opened = await request(port, {
            ...
            method: "POST",
            path: "/applets/usage-dashboard/open",
        });
        ...
        expect(openUrl.pathname).toBe("/applets/usage-dashboard/files/reports/daily.html");
        ...
        const unauthorizedContext = await request(port, {
            path: "/applets/usage-dashboard/context?token=wrong",
            token: "wrong",
        });
        expect(unauthorizedContext.status).toBe(401);
        const context = await request(port, {
            path: `/applets/usage-dashboard/context?token=${encodeURIComponent(contextToken!)}`,
            token: "wrong",
        });
        ...
        const favicon = await request(port, {
            path: "/applets/usage-dashboard/favicon.png",
        });
        ...
        const nested = await request(port, {
            path: "/applets/usage-dashboard/files/assets/app.js",
        });
        expect(nested.status).toBe(200);

        const dotfile = await request(port, { path: "/applets/usage-dashboard/files/.env" });
        expect(dotfile.status).toBe(400);
        // The WHATWG URL parser collapses encoded dot segments before routing, so an encoded
        // traversal lands back inside the applet instead of ever reaching the filesystem above it.
        const traversal = await request(port, {
            path: "/applets/usage-dashboard/files/assets/%2e%2e/index.html",
        });
        expect(traversal.body).toBe("<h1>one</h1>");
        await symlink(
            join(sources, "outside.txt"),
            join(process.env.HAPPY_APPLETS_DIRECTORY!, "usage-dashboard", "v1", "escape.txt"),
        );
        const symlinked = await request(port, {
            path: "/applets/usage-dashboard/files/escape.txt",
        });
        expect(symlinked.status).toBe(400);
        const unknownExtension = await request(port, {
            path: "/applets/usage-dashboard/files/index.exe",
        });
        expect(unknownExtension.status).toBe(404);
        const unknownApplet = await request(port, { path: "/applets/other/files/" });
        expect(unknownApplet.status).toBe(404);
        ...
        const missingChange = await request(port, {
            ...
            method: "POST",
            path: "/applets/usage-dashboard/versions",
        });
        ...
        expect(updated.status).toBe(200);
        expect(JSON.parse(updated.body)).toMatchObject({ applet: { currentVersion: 2 } });
        const second = await request(port, { path: "/applets/usage-dashboard/files/" });
        expect(second.body).toBe("<h1>two</h1>");

        const badRevert = await request(port, {
            body: JSON.stringify({ version: 9 }),
            method: "POST",
            path: "/applets/usage-dashboard/revert",
        });
        ...
        expect(JSON.parse(reverted.body)).toMatchObject({
            applet: { currentVersion: 1, versions: [{ version: 1 }, { version: 2 }] },
        });
        const first = await request(port, { path: "/applets/usage-dashboard/files/" });
        expect(first.body).toBe("<h1>one</h1>");

        const listed = await request(port, { path: "/applets" });
        expect(listed.status).toBe(200);
        expect(JSON.parse(listed.body)).toMatchObject({
            applets: [{ currentVersion: 1, name: "usage-dashboard" }],
        });
    });

    it("rejects opening an applet after its declaration narrows past an existing slot", async () => {
        const port = await startServer();
        const sources = await createTempDirectory("rig-applet-scopes-");
        ...
        const slot = await request(port, {
            body: JSON.stringify({
                author: { type: "agent", sessionId: "session-1" },
                content: {
                    action: { type: "open-applet", applet: "scoped-dashboard" },
        ...
        const narrowed = await request(port, {
            ...
            method: "POST",
            path: "/applets/scoped-dashboard/versions",
        });
        ...
        const opened = await request(port, {
            body: JSON.stringify({}),
            method: "POST",
            path: "/applets/scoped-dashboard/open",
        });
        ...
        expect(JSON.parse(opened.body)).toEqual({
            error: {
                code: "invalid_applet",
                message:
                    'The applet "scoped-dashboard" does not allow the everywhere scope. It allows only the project scope.',
            },
        });
    });
});
