...
Rig adds no account of its own. It uses the credentials already managed by the
...
- **Happy 2** is its desktop collaborative sibling: a self-hosted, Slack-like
  ...
  `.happy2`. It runs its agents on Rig: a private Rig runtime, each agent
  conversation bound to a sandboxed container, Rig sessions, terminals, and
...
| [DESIGN.md](DESIGN.md)                                     | The visual specification for a Happy plugin app: host style variables, surfaces, layout grid, typography, controls, states, and a copyable baseline. Read it before designing or building a Happy webapp or plugin UI. |
...
1. **Write a plugin** — TypeScript, one `happy.plugin.json` manifest, a
   ...
   sandboxed process, talks to Rig over an authenticated socket through the
...
4. **Change Rig itself** — Rig is developed with Rig. When you have the Rig
